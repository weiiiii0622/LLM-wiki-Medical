...
FOURTEENTH_BOOK_SEEDS: tuple[TopicSeed, ...] = (
    # Foot, ankle, knee, and lower-extremity trauma.
    TopicSeed("condition", "ankle-fracture", "Ankle fracture (踝關節骨折)", ("ankle fracture", "踝關節骨折", "malleolar fracture", "bimalleolar", "trimalleolar"), "踝關節骨折需評估內外踝、後踝、syndesmosis 與 talar shift；穩定無位移可石膏，位移或不穩定多需 ORIF。"),
    TopicSeed("diagnostic", "ottawa-ankle-rules", "Ottawa ankle rules (Ottawa 足踝 X 光規則)", ("Ottawa ankle", "Ottawa ankle rules", "malleolar zone", "midfoot zone"), "急性足踝傷害是否需 X-ray 的臨床規則，依 malleolar/midfoot tenderness 與負重能力判斷。"),
    TopicSeed("condition", "calcaneus-fracture", "Calcaneus fracture (跟骨骨折)", ("calcaneus fracture", "calcaneal fracture", "跟骨骨折", "Böhler", "Bohler"), "常由高處墜落造成，需注意 intra-articular involvement、Böhler angle 下降及合併腰椎或下肢骨折。"),
    TopicSeed("diagnostic", "bohler-angle", "Böhler angle (Böhler 角)", ("Böhler", "Bohler", "Böhler's angle", "Bohler's angle"), "跟骨側位 X-ray 評估 posterior facet 塌陷的角度，低於正常範圍提示 calcaneus fracture。"),
    TopicSeed("condition", "achilles-tendon-rupture", "Achilles tendon rupture (阿基里斯腱斷裂)", ("Achilles tendon rupture", "Achilles tendon", "阿基里斯腱撕裂", "Thompson test"), "常見於運動或突然蹬地，表現為後踝疼痛、跛行與 plantar flexion 無力，Thompson test 可輔助診斷。"),
    TopicSeed("condition", "anterior-cruciate-ligament-injury", "Anterior cruciate ligament injury, ACL injury (前十字韌帶傷害)", ("anterior cruciate ligament", "ACL", "前十字韌帶", "Lachman", "pivot shift"), "ACL injury 常有 pop 聲、急性血腫與膝不穩，Lachman test 敏感，年輕或高活動需求者常考慮重建。"),
    TopicSeed("condition", "posterior-cruciate-ligament-injury", "Posterior cruciate ligament injury, PCL injury (後十字韌帶傷害)", ("posterior cruciate ligament", "PCL", "後十字韌帶", "posterior drawer", "dashboard injury"), "PCL injury 常由 dashboard injury 或 hyperextension 造成，posterior drawer test 與 posterior sagging 有助診斷。"),
    TopicSeed("condition", "meniscus-tear", "Meniscus tear (半月板破裂)", ("meniscus tear", "meniscal tear", "半月板破裂", "McMurray", "discoid meniscus"), "半月板破裂常有關節線疼痛、locking、crepitus；MRI 與 arthroscopy 可診斷，治療依 tear pattern 與 vascular zone。"),
    TopicSeed("procedure", "acl-reconstruction", "ACL reconstruction (前十字韌帶重建術)", ("ACL reconstruction", "前十字韌帶重建", "bone patella tendon bone", "hamstring tendon graft"), "用自體或異體 graft 重建 ACL，適用於高度活動、complete rupture、不穩或合併半月板/側副韌帶傷害者。"),
    TopicSeed("procedure", "meniscus-repair", "Meniscus repair / meniscectomy (半月板修補／切除術)", ("meniscus repair", "meniscectomy", "半月板修補", "半月板切除", "meniscal transplantation"), "半月板手術依紅白區供血與裂傷型態選擇 repair、partial meniscectomy 或少見的 transplantation。"),
    TopicSeed("condition", "open-fracture", "Open fracture (開放性骨折)", ("open fracture", "開放性骨折", "Gustilo", "Gustilo-Anderson"), "骨折與外界傷口相通，屬骨科急症，處理核心是清洗清創、抗生素、固定與軟組織覆蓋。"),
    TopicSeed("guideline", "gustilo-anderson-open-fracture-classification", "Gustilo-Anderson open fracture classification (Gustilo 開放性骨折分類)", ("Gustilo", "Gustilo-Anderson", "Type IIIA", "Type IIIB", "Type IIIC"), "依傷口大小、污染、軟組織覆蓋與血管損傷分級開放性骨折，影響抗生素、清創與固定策略。"),
    TopicSeed("procedure", "fracture-reduction-fixation", "Fracture reduction and fixation (骨折復位與固定)", ("reduction and fixation", "open reduction", "closed reduction", "internal fixation", "external fixation", "復位", "內固定", "外固定"), "骨折治療包含 closed/open reduction 與 internal/external fixation，目的為恢復解剖、穩定骨折並減少軟組織傷害。"),
    TopicSeed("procedure", "open-reduction-internal-fixation", "Open reduction internal fixation, ORIF (開放復位內固定)", ("ORIF", "open reduction internal fixation", "open reduction and internal fixation", "骨折復位及內固定", "鋼板", "螺絲"), "手術暴露骨折端後復位並以 plate、screw、pin 或 wire 固定，常用於位移、不穩定或關節內骨折。"),
    TopicSeed("procedure", "external-fixation", "External fixation (外固定)", ("external fixation", "external fixator", "外固定器", "外固定"), "以皮外固定架跨越骨折或關節暫時/ definitivo 穩定，常用於開放性骨折、嚴重軟組織傷害或 damage-control orthopedics。"),
    TopicSeed("condition", "femoral-shaft-fracture", "Femoral shaft fracture (股骨幹骨折)", ("femoral shaft fracture", "股骨幹骨折", "traction pin", "intramedullary nail"), "高能量創傷常見，失血量可大，治療多為 intramedullary nailing，需注意脂肪栓塞、nonunion 與 malunion。"),
    TopicSeed("condition", "tibial-fibular-fracture", "Tibial-fibular fracture (脛腓骨骨折／小腿骨折)", ("tibial-fibula fracture", "tibial fibular fracture", "小腿骨折", "tibial shaft fracture", "proximal tibia fracture"), "脛腓骨骨折須評估開放傷、血管神經狀態與 compartment syndrome，治療依位置、穩定度與軟組織狀況。"),
    TopicSeed("condition", "compartment-syndrome", "Compartment syndrome (腔室症候群)", ("compartment syndrome", "腔室症候群", "fasciotomy", "pain out of proportion"), "筋膜室壓力上升造成肌肉神經缺血，疼痛超出預期與被動伸展痛是重要警訊，治療為緊急 fasciotomy。"),
    TopicSeed("procedure", "fasciotomy", "Fasciotomy (筋膜切開術)", ("fasciotomy", "筋膜切開", "筋膜切開術"), "以切開筋膜降低 compartment pressure，是急性 compartment syndrome 的 limb-saving procedure。"),
    # Hip, pelvis, upper limb, and hand.
    TopicSeed("condition", "femoral-neck-fracture", "Femoral neck fracture (股骨頸骨折)", ("femoral neck fracture", "股骨頸骨折", "Garden classification"), "老年人跌倒常見，需區分 intracapsular fracture 與位移程度，併發症包含 avascular necrosis 與 nonunion。"),
    TopicSeed("condition", "intertrochanteric-fracture", "Intertrochanteric fracture (股骨轉子間骨折)", ("intertrochanteric fracture", "轉子間骨折", "proximal femur fracture"), "股骨近端 extracapsular fracture，老年骨鬆患者常見，多需內固定並早期活動。"),
    TopicSeed("condition", "pelvic-fracture", "Pelvic fracture (骨盆骨折)", ("pelvic fracture", "骨盆骨折", "open-book", "vertical shear"), "高能量骨盆環損傷可造成大量出血、泌尿道損傷與 hemodynamic instability，需骨盆固定與止血策略。"),
    TopicSeed("condition", "osteonecrosis-femoral-head", "Osteonecrosis of femoral head, ONFH (股骨頭缺血性壞死)", ("osteonecrosis of femoral head", "ONFH", "股骨頭壞死", "avascular necrosis of femoral head", "FICAT"), "股骨頭血流受損造成壞死塌陷，風險包含 steroid、alcohol、trauma、SLE 與 sickle cell disease。"),
    TopicSeed("condition", "shoulder-dislocation", "Shoulder dislocation (肩關節脫位)", ("shoulder dislocation", "肩關節脱位", "肩關節脫位", "Bankart", "Hill-Sachs"), "肩關節最常見前脫位，需評估 axillary nerve、Bankart lesion 與 Hill-Sachs lesion，治療為復位與復健/穩定手術。"),
    TopicSeed("condition", "acromioclavicular-joint-injury", "Acromioclavicular joint injury (肩峰鎖骨關節傷害)", ("acromioclavicular injury", "AC joint injury", "肩峰鎖骨關節傷害", "Rockwood"), "跌倒撞擊肩峰可造成 AC joint separation，Rockwood 分型依韌帶斷裂與位移決定保守或手術。"),
    TopicSeed("condition", "brachial-plexus-injury", "Brachial plexus injury (臂神經叢損傷)", ("brachial plexus", "臂神經叢", "Erb palsy", "Klumpke"), "臂神經叢牽拉或撕裂可造成上肢運動與感覺缺損，定位需依 root/trunk/cord/terminal branch。"),
    TopicSeed("anatomy", "brachial-plexus", "Brachial plexus (臂神經叢)", ("brachial plexus", "臂神經叢", "lateral cord", "posterior cord", "medial cord"), "由 C5-T1 roots 形成 trunks、divisions、cords 與 terminal branches，支配上肢運動與感覺。"),
    TopicSeed("condition", "rotator-cuff-injury", "Rotator cuff injury (旋轉肌群損傷)", ("rotator cuff injury", "rotator cuff tear", "旋轉肌群受傷", "supraspinatus", "subscapularis"), "常侵犯 supraspinatus，造成肩痛、夜痛與外展無力，診斷依理學檢查、超音波或 MRI。"),
    TopicSeed("condition", "supracondylar-humerus-fracture", "Supracondylar humerus fracture (肱骨上髁骨折)", ("supracondylar fracture", "肱骨上髁骨折", "Volkmann", "anterior interosseous nerve"), "兒童肘部常見骨折，需評估 brachial artery、median/AIN injury 與 compartment syndrome。"),
    TopicSeed("condition", "epicondylitis", "Epicondylitis (肱骨上髁炎)", ("epicondylitis", "肱骨上踝炎", "肱骨上髁炎", "tennis elbow", "golfer elbow"), "伸腕或屈腕肌腱附著處 overuse tendinopathy，分 lateral epicondylitis 與 medial epicondylitis。"),
    TopicSeed("condition", "elbow-dislocation", "Elbow dislocation (手肘脫位)", ("elbow dislocation", "手肘脱位", "手肘脫位"), "成人常見大關節脫位，復位後需評估穩定度、神經血管與合併骨折。"),
    TopicSeed("condition", "distal-radius-fracture", "Distal radius fracture (遠端橈骨骨折)", ("distal radius fracture", "遠端橈骨骨折", "Colles fracture", "Smith fracture", "volar tilt"), "跌倒手撐地常見，影像需評估 radial height、radial inclination、volar tilt 與關節面；常見型態包含 Colles 與 Smith。"),
    TopicSeed("condition", "carpal-tunnel-syndrome", "Carpal tunnel syndrome (腕隧道症候群)", ("carpal tunnel syndrome", "腕隧道症候群", "median nerve", "Phalen", "Tinel"), "正中神經於腕隧道壓迫造成拇、食、中指麻痛與 thenar weakness，檢查包含 Phalen/Tinel 與 nerve conduction study。"),
    TopicSeed("condition", "scaphoid-fracture", "Scaphoid fracture (舟狀骨骨折)", ("scaphoid fracture", "舟狀骨骨折", "anatomic snuffbox"), "跌倒手撐地後鼻煙窩壓痛需懷疑，近端血供差，延誤可導致 nonunion 與 avascular necrosis。"),
    TopicSeed("condition", "kienbock-disease", "Kienböck disease (月狀骨缺血性壞死)", ("Kienböck", "Kienbock", "lunate bone AVN", "半月狀骨缺血性壞死", "月狀骨缺血性壞死"), "月狀骨血流不足造成手腕疼痛與塌陷，與 ulnar negative variance 等因素相關。"),
    TopicSeed("condition", "extensor-tendon-injury-hand", "Extensor tendon injury of hand (手部伸肌肌腱損傷)", ("extensor tendon injury", "伸肌肌腱傷害", "mallet finger", "boutonniere"), "手部伸肌腱損傷可依 zone 造成 mallet finger、boutonniere deformity 或伸指障礙，治療依位置與斷裂程度。"),
    TopicSeed("condition", "de-quervain-tenosynovitis", "De Quervain tenosynovitis (狄奎凡氏腱鞘炎)", ("de Quervain", "Quervain", "狄奎凡", "Finkelstein", "extensor pollicis brevis"), "第一背側腕伸肌腱鞘發炎，造成橈側手腕痛，Finkelstein test 可誘發疼痛。"),
    TopicSeed("condition", "trigger-finger", "Trigger finger (板機指)", ("trigger finger", "板機指", "A1 pulley", "stenosing tenosynovitis"), "屈指肌腱於 A1 pulley 狹窄卡住，造成手指彈響、卡鎖或伸直困難。"),
    # Bone tumors and metabolic/pediatric orthopedics.
    TopicSeed("condition", "bone-metastasis", "Bone metastasis (骨轉移)", ("bone metastasis", "metastatic bone tumor", "骨轉移", "osteolytic lesion", "osteoblastic lesion"), "成人惡性骨病灶常為轉移癌，常見來源包含肺、乳、攝護腺、甲狀腺與肝癌，需評估病理性骨折與 spinal cord compression。"),
    TopicSeed("condition", "osteoid-osteoma", "Osteoid osteoma (骨樣骨瘤)", ("osteoid osteoma", "骨樣骨瘤", "nidus", "night pain"), "良性骨性腫瘤，常見於年輕人長骨骨幹，典型夜痛且對 NSAID 反應佳，影像見小 nidus 與 reactive sclerosis。"),
    TopicSeed("condition", "osteoblastoma", "Osteoblastoma (骨母細胞瘤)", ("osteoblastoma", "骨母細胞瘤"), "較具侵犯性的良性骨腫瘤，常見於脊椎 posterior elements，nidus 較 osteoid osteoma 大，可能壓迫神經。"),
    TopicSeed("condition", "fibrous-dysplasia", "Fibrous dysplasia (纖維性發育不良)", ("fibrous dysplasia", "纖維性發育不良", "ground-glass", "shepherd's crook"), "骨發育異常使 lamellar bone 被纖維骨組織取代，影像可見 ground-glass appearance，近端股骨可有 shepherd's crook deformity。"),
    TopicSeed("condition", "osteochondroma", "Osteochondroma (骨軟骨瘤)", ("osteochondroma", "骨軟骨瘤", "exostosis", "multiple hereditary exostosis"), "最常見良性骨腫瘤，長於 metaphysis 表面並與骨髓腔連通；多發型有較高惡性轉化風險。"),
    TopicSeed("condition", "enchondroma", "Enchondroma (內生軟骨瘤)", ("enchondroma", "內生軟骨瘤", "chondroma", "Ollier", "Maffucci"), "骨髓腔內良性軟骨腫瘤，常見於手骨，可合併病理性骨折；多發需注意惡性轉化。"),
    TopicSeed("condition", "chondroblastoma", "Chondroblastoma (軟骨母細胞瘤)", ("chondroblastoma", "軟骨母細胞瘤"), "年輕人 epiphysis 良性軟骨腫瘤，可造成關節附近疼痛與 lytic lesion。"),
    TopicSeed("condition", "giant-cell-tumor-bone", "Giant cell tumor of bone (骨巨細胞瘤)", ("giant cell tumor", "GCT", "骨巨細胞瘤", "soap bubble"), "好發骨骺閉合後 epiphysis/metaphysis，影像可呈 soap-bubble lytic lesion，局部侵襲且可復發。"),
    TopicSeed("condition", "chondrosarcoma", "Chondrosarcoma (軟骨肉瘤)", ("chondrosarcoma", "軟骨肉瘤"), "成人常見原發惡性骨腫瘤之一，源自軟骨，常見於骨盆、肩帶或長骨近端，治療以手術為主。"),
    TopicSeed("condition", "simple-bone-cyst", "Simple bone cyst / unicameral bone cyst (單腔骨囊腫)", ("simple bone cyst", "unicameral bone cyst", "單腔骨囊腫", "fallen fragment"), "兒童長骨 metaphysis 良性囊性病灶，常因病理性骨折發現，影像可有 fallen fragment sign。"),
    TopicSeed("condition", "aneurysmal-bone-cyst", "Aneurysmal bone cyst (動脈瘤樣骨囊腫)", ("aneurysmal bone cyst", "ABC", "動脈瘤樣骨囊腫", "fluid-fluid level"), "血液充滿的膨脹性良性骨病灶，MRI 可見 fluid-fluid levels，可原發或繼發於其他腫瘤。"),
    TopicSeed("diagnostic", "bone-tumor-location-framework", "Bone tumor location framework (骨腫瘤好發部位架構)", ("epiphysis", "metaphysis", "diaphysis", "SOFA", "FALLOMMEE", "骨腫瘤好發"), "以 epiphysis、metaphysis、diaphysis 分布協助骨腫瘤鑑別診斷，需結合年齡、影像與病理。"),
    TopicSeed("diagnostic", "bone-tumor-radiographic-signs", "Bone tumor radiographic signs (骨腫瘤影像徵象)", ("Codman's triangle", "sunburst", "onion-skin", "soap bubble", "ground-glass", "lytic lesion"), "骨腫瘤 X-ray 徵象包含 periosteal reaction、lytic/blastic pattern、matrix calcification 與 cortex destruction。"),
    TopicSeed("condition", "congenital-radioulnar-synostosis", "Congenital radioulnar synostosis (先天性橈尺骨融合)", ("congenital radioulnar synostosis", "先天性橈尺骨融合"), "近端橈尺骨先天融合造成前臂旋前/旋後受限，常雙側且可合併其他先天異常。"),
    TopicSeed("condition", "slipped-capital-femoral-epiphysis", "Slipped capital femoral epiphysis, SCFE (股骨頭骨骺滑脫)", ("slipped capital femoral epiphysis", "SCFE", "股骨生長板滑脱", "股骨頭骨骺滑脫"), "青少年近端股骨骨骺經生長板滑脫，常見於肥胖男孩或內分泌異常，表現為髖/膝痛與外旋步態。"),
    TopicSeed("condition", "developmental-dysplasia-of-hip", "Developmental dysplasia of hip, DDH (髖關節發育不良)", ("developmental dysplasia of hip", "DDH", "髋關節發育不良", "髖關節發育不良", "Barlow", "Ortolani"), "嬰兒髖臼與股骨頭關係異常，可由 Barlow/Ortolani、超音波或 X-ray 評估，治療依年齡用 Pavlik harness、復位或手術。"),
    TopicSeed("condition", "legg-calve-perthes-disease", "Legg-Calve-Perthes disease (小兒股骨頭缺血性壞死)", ("Legg", "Calve", "Perthes", "LCPD", "小兒股骨頭缺血性壞死"), "兒童股骨頭暫時性血流不足造成壞死與重塑，常見於男童，治療目標是保持 femoral head containment。"),
)


FIFTEENTH_BOOK_SEEDS: tuple[TopicSeed, ...] = (
    # Obstetric physiology, screening, fetal assessment, and antepartum complications.
    TopicSeed("physiology", "maternal-physiologic-changes-pregnancy", "Maternal physiologic changes in pregnancy (妊娠母體生理變化)", ("maternal-fetal physiology", "產婦胎兒生理學", "妊娠時生理", "懷孕時生理", "supine hypotension"), "妊娠造成心輸出量、血容量、腎血流/GFR、呼吸通氣、凝血與皮膚/胃腸泌尿系統變化，影響正常檢驗值與臨床處置。"),
    TopicSeed("physiology", "fetal-circulation", "Fetal circulation (胎兒循環)", ("fetal circulation", "胎兒心血管", "ductus venosus", "ductus arteriosus", "foramen ovale", "umbilical vein"), "胎兒以臍靜脈、靜脈導管、卵圓孔與動脈導管分流氧合血；出生後肺循環開啟並關閉 fetal shunts。"),
    TopicSeed("condition", "twin-pregnancy", "Twin pregnancy (雙胞胎妊娠)", ("twin pregnancy", "雙胞胎妊娠", "dichorionic", "monochorionic", "monoamniotic"), "雙胎風險受 chorionicity、amnionicity 與單胎死亡、TTTS、早產、生長不一致等因素影響。"),
    TopicSeed("condition", "twin-twin-transfusion-syndrome", "Twin-twin transfusion syndrome, TTTS (雙胞胎輸血症候群)", ("twin-twin transfusion", "TTTS", "雙胞胎輸血症候群", "donor twin", "recipient twin"), "單絨毛膜雙胎胎盤血管吻合造成一胎羊水過少、一胎羊水過多與血流動力不平衡。"),
    TopicSeed("diagnostic", "prenatal-care-schedule", "Prenatal care schedule (產檢時程)", ("prenatal examination", "產檢", "產檢時程", "健保產檢"), "產前照護依孕週安排病史、理學檢查、血尿檢、超音波、感染與遺傳篩檢。"),
    TopicSeed("diagnostic", "amniocentesis", "Amniocentesis (羊膜穿刺)", ("amniocentesis", "羊膜穿刺"), "以穿刺取得羊水做染色體、基因或感染檢測；需依孕週、適應症與流產風險評估。"),
    TopicSeed("diagnostic", "down-syndrome-prenatal-screening", "Down syndrome prenatal screening (唐氏症產前篩檢)", ("Down syndrome screening", "唐氏症篩檢", "nuchal translucency", "NIPT", "quadruple test"), "結合母血標記、nuchal translucency、NIPT 或診斷性檢查評估 trisomy 21 風險。"),
    TopicSeed("diagnostic", "group-b-streptococcus-screening-pregnancy", "Group B Streptococcus screening in pregnancy (妊娠乙型鏈球菌篩檢)", ("GBS", "乙型鏈球菌篩檢", "Group B Streptococcus", "intrapartum prophylaxis"), "妊娠後期篩檢 GBS colonization，陽性或高風險者生產時給予抗生素以預防新生兒感染。"),
    TopicSeed("condition", "fetal-growth-restriction", "Fetal growth restriction, FGR/IUGR (胎兒生長受限)", ("fetal growth restriction", "FGR", "IUGR", "胎兒生長遲滯", "胎兒生長受限"), "胎兒未達應有生長潛能，可分 symmetric/asymmetric，需用胎兒大小、羊水與 Doppler 追蹤胎盤功能。"),
    TopicSeed("diagnostic", "nonstress-test", "Nonstress test, NST (無壓力試驗)", ("nonstress test", "NST", "無壓力試驗", "reactive NST", "non-reactive NST"), "以胎心率加速反映胎兒氧合與自主神經反應，是常用 antepartum fetal surveillance。"),
    TopicSeed("diagnostic", "biophysical-profile", "Biophysical profile, BPP (胎兒生理活動評量)", ("biophysical profile", "BPP", "胎兒生理活動評量"), "整合 NST、胎動、呼吸運動、肌張力與羊水量評估胎兒狀態。"),
    TopicSeed("diagnostic", "umbilical-artery-doppler", "Umbilical artery Doppler (臍動脈都卜勒)", ("umbilical artery Doppler", "Doppler sonography", "S/D ratio", "臍動脈都卜勒", "absent end-diastolic flow"), "用臍動脈阻力與舒張末期血流評估胎盤阻力，常用於 FGR 監測與分娩時機判斷。"),
    TopicSeed("condition", "antepartum-hemorrhage", "Antepartum hemorrhage (產前出血)", ("antepartum hemorrhage", "產前出血"), "妊娠後半期陰道出血需鑑別 placenta previa、placental abruption、vasa previa、子宮頸/陰道病灶等。"),
    TopicSeed("condition", "placental-abruption", "Placental abruption (胎盤早期剝離)", ("placental abruption", "胎盤早期剝離", "Couvelaire uterus"), "胎盤於胎兒娩出前自子宮壁剝離，可造成疼痛性出血、子宮壓痛、胎兒窘迫與 DIC。"),
    TopicSeed("condition", "placenta-previa", "Placenta previa (前置胎盤)", ("placenta previa", "placental previa", "前置胎盤"), "胎盤覆蓋或接近內子宮頸口，典型為無痛性陰道出血，診斷依超音波，通常避免陰道內診。"),
    TopicSeed("condition", "placenta-accreta-spectrum", "Placenta accreta spectrum (植入性胎盤光譜)", ("placenta accreta", "placenta increta", "placenta percreta", "植入性胎盤"), "胎盤絨毛異常侵入子宮肌層或周邊器官，與前置胎盤和剖腹產史相關，可能造成嚴重產後出血。"),
    TopicSeed("condition", "preterm-birth", "Preterm birth (早產)", ("preterm birth", "早產", "preterm labor", "早期陣痛", "preterm rupture of membrane"), "妊娠未滿 37 週出生，常由 preterm labor 或 PPROM 引起，風險評估包含病史、子宮頸長度與 fetal fibronectin。"),
    TopicSeed("condition", "preterm-premature-rupture-of-membranes", "Preterm premature rupture of membranes, PPROM (早期破水)", ("PPROM", "preterm rupture of membrane", "premature rupture of membrane", "早期破水", "早期破膜"), "未足月且陣痛前胎膜破裂，處置需平衡感染、早產與胎兒肺成熟。"),
    TopicSeed("diagnostic", "fetal-fibronectin", "Fetal fibronectin (胎兒纖維連結蛋白)", ("fetal fibronectin", "胎兒纖維連結蛋白", "fFN"), "陰道/子宮頸分泌物 fFN 可輔助預測近期早產風險，陰性預測價值較高。"),
    TopicSeed("procedure", "tocolysis", "Tocolysis (安胎治療)", ("tocolysis", "安胎", "tocolytic", "ritodrine", "nifedipine", "indomethacin", "atosiban"), "短期抑制宮縮以爭取轉院或 antenatal corticosteroid 時間，需注意禁忌症與藥物副作用。"),
    TopicSeed("drug", "magnesium-sulfate-obstetrics", "Magnesium sulfate in obstetrics (產科硫酸鎂)", ("MgSO", "magnesium sulfate", "硫酸鎂", "eclampsia", "preeclampsia"), "用於子癲前症/子癲症 seizure prophylaxis/treatment，也可用於早產神經保護；需監測反射、呼吸與尿量。"),
    TopicSeed("condition", "hypertensive-disorders-of-pregnancy", "Hypertensive disorders of pregnancy (妊娠高血壓疾病)", ("pregnancy-induced hypertension", "妊娠高血壓", "gestational hypertension", "hypertensive disorders of pregnancy"), "妊娠相關高血壓包含 gestational hypertension、preeclampsia、eclampsia 與 chronic HTN superimposed preeclampsia。"),
    TopicSeed("condition", "preeclampsia", "Preeclampsia (子癲前症)", ("preeclampsia", "pre-eclampsia", "子癲前症", "severe features"), "20 週後新發高血壓合併蛋白尿或器官功能異常，可有 severe features；根本治療是分娩。"),
    TopicSeed("condition", "eclampsia", "Eclampsia (子癲症)", ("eclampsia", "子癲症"), "子癲前症病人出現無其他原因可解釋的癲癇發作，急性處置包含 MgSO4、穩定母體與適時分娩。"),
    TopicSeed("condition", "hellp-syndrome", "HELLP syndrome (HELLP 症候群)", ("HELLP", "HELLP syndrome", "溶血", "肝酵素上升", "血小板低下"), "preeclampsia spectrum 中的 hemolysis、elevated liver enzymes、low platelets，可迅速惡化並需分娩評估。"),
    TopicSeed("condition", "thyroid-disease-in-pregnancy", "Thyroid disease in pregnancy (妊娠合併甲狀腺疾病)", ("pregnancy with thyroid", "懷孕併甲狀腺", "妊娠甲狀腺", "Graves pregnancy"), "妊娠會改變 thyroid-binding globulin、hCG 對 TSH 的影響與藥物選擇，需區分 hyperthyroidism/hypothyroidism 與 fetal risk。"),
    # Labor, delivery, postpartum, and obstetric procedures.
    TopicSeed("physiology", "labor-mechanism", "Labor mechanism (分娩機轉)", ("labor mechanism", "cardinal movements", "分娩機轉", "engagement", "descent", "internal rotation"), "胎兒經產道的 cardinal movements 包含 engagement、descent、flexion、internal rotation、extension、external rotation 與 expulsion。"),
    TopicSeed("diagnostic", "bishop-score", "Bishop score (Bishop 子宮頸成熟度評分)", ("Bishop score", "Bishop", "子宮頸成熟度"), "以 dilation、effacement、station、consistency、position 評估 induction of labor 成功機率。"),
    TopicSeed("procedure", "induction-of-labor", "Induction of labor (引產)", ("induction of labor", "引產", "cervical ripening", "misoprostol", "dinoprostone"), "以 mechanical 或 pharmacologic 方法誘發產程，需依 Bishop score、胎兒狀態與禁忌症選擇。"),
    TopicSeed("drug", "oxytocin-obstetrics", "Oxytocin in obstetrics (產科 oxytocin)", ("oxytocin", "催產素", "Pitocin"), "用於 induction/augmentation of labor 與產後子宮收縮；需監測 uterine tachysystole 與 fetal heart tracing。"),
    TopicSeed("procedure", "cesarean-delivery", "Cesarean delivery (剖腹產)", ("cesarean delivery", "cesarean section", "C-section", "剖腹產", "剖宮產"), "經腹壁與子宮切開娩出胎兒，適應症包含胎兒窘迫、產程遲滯、胎位異常、前置胎盤等。"),
    TopicSeed("procedure", "operative-vaginal-delivery", "Operative vaginal delivery (器械輔助陰道生產)", ("operative vaginal delivery", "vacuum extraction", "forceps", "器械輔助", "真空吸引", "產鉗"), "使用 vacuum 或 forceps 輔助第二產程娩出，需符合頭位、子宮頸全開、胎頭位置明確等條件。"),
    TopicSeed("condition", "shoulder-dystocia", "Shoulder dystocia (肩難產)", ("shoulder dystocia", "肩難產", "McRoberts", "suprapubic pressure"), "胎頭娩出後肩膀卡住造成急症，處置包含 McRoberts maneuver、suprapubic pressure 與內旋/取後臂等。"),
    TopicSeed("condition", "fetal-heart-rate-decelerations", "Fetal heart rate decelerations (胎心率減速)", ("late deceleration", "variable deceleration", "early deceleration", "胎心率減速", "fetal heart rate"), "胎心監測減速型態可反映 head compression、cord compression 或 uteroplacental insufficiency。"),
    TopicSeed("condition", "postpartum-hemorrhage", "Postpartum hemorrhage, PPH (產後大出血)", ("postpartum hemorrhage", "PPH", "產後出血", "產後大出血", "uterine atony"), "產後出血常因 4T：tone、trauma、tissue、thrombin；需快速復甦、按摩、uterotonics、止血處置。"),
    TopicSeed("condition", "amniotic-fluid-embolism", "Amniotic fluid embolism (羊水栓塞)", ("amniotic fluid embolism", "羊水栓塞", "AFE"), "分娩或產後突發低氧、低血壓、DIC 的罕見高死亡率產科急症。"),
    TopicSeed("condition", "puerperal-infection", "Puerperal infection (產褥感染)", ("puerperal infection", "postpartum infection", "產褥感染", "endometritis postpartum"), "產後子宮內膜、傷口、泌尿道或乳房感染，可因剖腹產、破水久、內診多次等增加風險。"),
    TopicSeed("condition", "mastitis-lactational", "Lactational mastitis (哺乳性乳腺炎)", ("mastitis", "乳腺炎", "lactational mastitis", "breast abscess"), "哺乳期乳房疼痛、紅腫與發燒，多與乳汁鬱積和 Staphylococcus aureus 相關，需持續排乳與抗生素評估。"),
    # Gynecologic anatomy, benign disease, infections, and oncology.
    TopicSeed("anatomy", "female-reproductive-anatomy", "Female reproductive anatomy (女性生殖道解剖)", ("female reproductive anatomy", "婦科解剖", "子宫", "輸卵管", "卵巢", "陰道穹隆"), "女性生殖道包含外陰、陰道、子宮頸、子宮、輸卵管、卵巢與骨盆血管神經淋巴，影響手術與癌症轉移路徑。"),
    TopicSeed("condition", "pelvic-inflammatory-disease", "Pelvic inflammatory disease, PID (骨盆腔發炎性疾病)", ("pelvic inflammatory disease", "PID", "骨盆腔發炎", "Fitz-Hugh-Curtis"), "上生殖道感染造成子宮內膜炎、輸卵管炎或骨盆腹膜炎，可導致慢性骨盆痛、不孕與 ectopic pregnancy。"),
    TopicSeed("condition", "tubo-ovarian-abscess", "Tubo-ovarian abscess, TOA (卵巢輸卵管膿瘍)", ("tubo-ovarian abscess", "TOA", "卵巢輸卵管膿瘍"), "PID 併發膿瘍形成，可能需廣效抗生素、影像導引引流或手術。"),
    TopicSeed("condition", "endometriosis", "Endometriosis (子宮內膜異位症)", ("endometriosis", "子宮內膜異位", "endometrioma", "chocolate cyst"), "子宮內膜樣組織在子宮外生長，造成 dysmenorrhea、dyspareunia、chronic pelvic pain、不孕與 endometrioma。"),
    TopicSeed("condition", "adenomyosis", "Adenomyosis (子宮腺肌症)", ("adenomyosis", "子宮腺肌症"), "子宮內膜腺體與基質進入肌層造成子宮瀰漫性增大、經痛與經血過多。"),
    TopicSeed("condition", "uterine-leiomyoma", "Uterine leiomyoma / myoma (子宮肌瘤)", ("uterine myoma", "uterine leiomyoma", "fibroid", "子宮肌瘤"), "子宮平滑肌良性腫瘤，可造成經血過多、壓迫症狀、不孕或妊娠併發症，治療依症狀與生育需求。"),
    TopicSeed("condition", "abnormal-uterine-bleeding", "Abnormal uterine bleeding, AUB (異常子宮出血)", ("abnormal uterine bleeding", "AUB", "異常子宮出血", "menorrhagia", "metrorrhagia"), "非正常頻率、量、規律或持續時間的子宮出血，常用 PALM-COEIN 分類。"),
    TopicSeed("condition", "endometrial-hyperplasia", "Endometrial hyperplasia (子宮內膜增生)", ("endometrial hyperplasia", "子宮內膜增生", "atypical hyperplasia"), "雌激素未受黃體素拮抗造成內膜增生，非典型增生與 endometrial cancer 風險增加。"),
    TopicSeed("condition", "endometrial-cancer", "Endometrial cancer (子宮內膜癌)", ("endometrial cancer", "子宮內膜癌", "endometrioid carcinoma"), "常以停經後出血表現，風險與 unopposed estrogen、obesity、PCOS、Lynch syndrome 相關。"),
    TopicSeed("condition", "ovarian-cyst", "Ovarian cyst (卵巢囊腫)", ("ovarian cyst", "卵巢囊腫", "follicular cyst", "corpus luteum cyst", "theca lutein cyst"), "卵巢囊性病灶多為功能性或良性，但需依年齡、超音波特徵、CA-125 與症狀評估惡性和扭轉/破裂風險。"),
    TopicSeed("condition", "ovarian-torsion", "Ovarian torsion (卵巢扭轉)", ("ovarian torsion", "adnexal torsion", "卵巢扭轉", "腫瘤扭轉"), "卵巢或附件血管扭轉造成急性下腹痛、噁心嘔吐，是需手術評估的婦科急症。"),
    TopicSeed("condition", "epithelial-ovarian-cancer", "Epithelial ovarian cancer (上皮性卵巢癌)", ("epithelial ovarian cancer", "上皮性卵巢癌", "high-grade serous", "CA125", "HE4"), "最常見卵巢惡性腫瘤，常晚期診斷；評估包含超音波、CA-125/HE4、CT 與 surgical staging/debulking。"),
    TopicSeed("condition", "ovarian-germ-cell-tumor", "Ovarian germ cell tumor (卵巢生殖細胞腫瘤)", ("ovarian germ cell tumor", "卵巢生殖細胞腫瘤", "dysgerminoma", "yolk sac tumor", "immature teratoma"), "多見於年輕女性，腫瘤標記可包含 AFP、beta-hCG、LDH，治療需兼顧 fertility preservation。"),
    TopicSeed("condition", "ovarian-sex-cord-stromal-tumor", "Ovarian sex-cord stromal tumor (卵巢性索間質腫瘤)", ("sex-cord stromal tumor", "性索間質腫瘤", "granulosa cell tumor", "Sertoli-Leydig"), "可分泌 estrogen 或 androgen，表現為 abnormal bleeding、precocious puberty 或 virilization。"),
    TopicSeed("condition", "mature-cystic-teratoma", "Mature cystic teratoma / dermoid cyst (成熟囊性畸胎瘤)", ("mature cystic teratoma", "dermoid cyst", "成熟囊性畸胎瘤", "皮樣囊腫"), "最常見卵巢生殖細胞良性腫瘤，含外胚層成分，常與 ovarian torsion 相關。"),
    TopicSeed("condition", "krukenberg-tumor", "Krukenberg tumor (Krukenberg 轉移性卵巢腫瘤)", ("Krukenberg", "Krukenberg tumor", "圖章戒指", "signet-ring"), "胃腸道等癌症轉移至卵巢的腫瘤，典型 signet-ring cells，預後差。"),
    TopicSeed("procedure", "ovarian-cancer-debulking-surgery", "Ovarian cancer debulking surgery (卵巢癌減積手術)", ("debulking", "optimal debulking", "減積手術", "staging surgery"), "卵巢癌手術目標為 staging 與最大程度 cytoreduction，optimal debulking 影響預後。"),
    TopicSeed("diagnostic", "ca-125", "CA-125 (癌抗原 125)", ("CA125", "CA-125", "癌抗原 125"), "卵巢上皮癌常用追蹤與輔助評估標記，但早期、停經前與非漿液性癌診斷準確度有限。"),
    TopicSeed("condition", "human-papillomavirus-infection", "Human papillomavirus infection, HPV (人類乳突病毒感染)", ("HPV", "human papillomavirus", "人類乳突病毒", "HPV 16", "HPV 18"), "HPV 高風險型與 cervical intraepithelial lesion、cervical cancer 及部分陰道/外陰癌相關。"),
    TopicSeed("drug", "hpv-vaccine", "HPV vaccine (HPV 疫苗)", ("HPV vaccine", "子宮頸疫苗", "HPV 疫苗", "Gardasil", "Cervarix"), "預防高風險 HPV 感染與相關癌前病變/癌症，接種建議依年齡與地區 guideline。"),
    TopicSeed("condition", "cervical-intraepithelial-neoplasia", "Cervical intraepithelial neoplasia, CIN (子宮頸癌前病變)", ("CIN", "cervical intraepithelial", "子宮頸癌前病變", "LSIL", "HSIL"), "HPV 相關子宮頸上皮內病變，依 cytology、HPV testing、colposcopy 與 biopsy 風險分層處置。"),
    TopicSeed("condition", "cervical-cancer", "Cervical cancer (子宮頸癌)", ("cervical cancer", "子宮頸癌", "squamous cell carcinoma cervix", "adenocarcinoma cervix"), "多與高風險 HPV 相關，篩檢可早期發現 CIN；治療依 FIGO stage 選擇手術、放化療。"),
    TopicSeed("diagnostic", "pap-smear", "Pap smear / cervical cytology (子宮頸抹片)", ("Pap smear", "Papanicolaou", "子宮頸抹片", "cervical cytology"), "子宮頸癌篩檢工具，偵測 cytologic abnormality，常與 HPV testing 合併。"),
    TopicSeed("diagnostic", "colposcopy", "Colposcopy (陰道鏡)", ("colposcopy", "陰道鏡", "acetowhite", "transformation zone"), "放大檢視子宮頸轉化區並導引 biopsy，用於異常抹片或 HPV 風險評估。"),
    TopicSeed("procedure", "cervical-conization", "Cervical conization / LEEP (子宮頸錐狀切除)", ("conization", "LEEP", "子宮頸錐狀切除", "loop electrosurgical excision"), "切除子宮頸轉化區病灶以診斷或治療 HSIL/CIN，需注意出血、感染與早產風險。"),
    TopicSeed("condition", "vaginal-cancer", "Vaginal cancer (陰道癌)", ("vaginal cancer", "陰道癌"), "少見婦癌，多與 HPV、子宮頸癌病史或放射線相關，需與子宮頸/外陰/轉移病灶區分。"),
    TopicSeed("condition", "vulvar-cancer", "Vulvar cancer (外陰癌)", ("vulvar cancer", "外陰癌", "vulvar squamous cell carcinoma"), "多為 squamous cell carcinoma，與 HPV 或 lichen sclerosus 相關，治療依局部切除與淋巴評估。"),
    TopicSeed("condition", "gestational-trophoblastic-disease", "Gestational trophoblastic disease, GTD (妊娠滋養層疾病)", ("gestational trophoblastic disease", "GTD", "妊娠滋養層疾病", "hydatidiform mole"), "滋養層異常增生疾病群，包含 complete/partial mole、invasive mole、choriocarcinoma，追蹤核心是 beta-hCG。"),
    TopicSeed("condition", "choriocarcinoma", "Choriocarcinoma (絨毛膜癌)", ("choriocarcinoma", "絨毛膜癌", "malignant GTD"), "高度惡性滋養層腫瘤，血行轉移常見但對化療敏感，需依 hCG 與 FIGO/WHO risk score 治療。"),
    # Reproductive endocrinology, infertility, urogynecology, and endoscopy.
    TopicSeed("condition", "amenorrhea", "Amenorrhea (無月經)", ("amenorrhea", "無月經", "primary amenorrhea", "secondary amenorrhea"), "月經未來或停止，需依 pregnancy、outflow tract、uterine、ovarian、pituitary、hypothalamic causes 評估。"),
    TopicSeed("condition", "primary-ovarian-insufficiency", "Primary ovarian insufficiency, POI (原發性卵巢功能不全)", ("primary ovarian insufficiency", "POI", "原發性卵巢功能不全", "premature ovarian failure"), "40 歲前卵巢功能衰退，表現為 amenorrhea、高 FSH 與低 estrogen，需評估遺傳、自體免疫與骨心血管風險。"),
    TopicSeed("condition", "menopause", "Menopause (更年期／停經)", ("menopause", "更年期", "停經", "vasomotor symptoms"), "卵巢濾泡耗竭造成月經永久停止與 estrogen 下降，可有熱潮紅、泌尿生殖症候群與骨質流失。"),
    TopicSeed("drug", "menopausal-hormone-therapy", "Menopausal hormone therapy, MHT (更年期荷爾蒙治療)", ("menopausal hormone therapy", "MHT", "更年期荷爾蒙", "hormone therapy"), "用 estrogen +/- progestin 改善更年期症狀與骨質，但需依子宮狀態、年齡、血栓/乳癌/心血管風險評估。"),
    TopicSeed("condition", "recurrent-pregnancy-loss", "Recurrent pregnancy loss (反覆性流產)", ("recurrent pregnancy loss", "RPL", "反覆性流產", "habitual abortion"), "反覆自然流產需評估染色體、子宮結構、APS、內分泌、感染與生活因素。"),
    TopicSeed("condition", "female-infertility", "Female infertility (女性不孕症)", ("female infertility", "女性不孕", "女性不孕症", "anovulation infertility", "tubal factor infertility"), "女性因素不孕包含排卵、輸卵管、子宮、子宮內膜異位與年齡/卵巢庫存問題。"),
    TopicSeed("diagnostic", "infertility-evaluation", "Infertility evaluation (不孕症評估)", ("infertility evaluation", "不孕症評估", "basal body temperature", "ovulation predictor", "AMH", "AFC", "HSG"), "不孕評估整合排卵、卵巢庫存、輸卵管通暢、子宮腔、男性精液與年齡/病史風險。"),
    TopicSeed("diagnostic", "hysterosalpingography", "Hysterosalpingography, HSG (子宮輸卵管攝影)", ("hysterosalpingography", "HSG", "子宮輸卵管攝影"), "以顯影劑評估子宮腔形態與輸卵管通暢，是 infertility/tubal factor 常用檢查。"),
    TopicSeed("procedure", "assisted-reproductive-technology", "Assisted reproductive technology, ART (人工協助生殖技術)", ("assisted reproductive technique", "assisted reproductive technology", "ART", "人工協助生殖", "IVF", "ICSI", "IUI"), "包含 IUI、IVF、ICSI、胚胎培養與植入等技術，適用於輸卵管、男性、排卵、年齡或 unexplained infertility。"),
    TopicSeed("procedure", "in-vitro-fertilization", "In vitro fertilization, IVF (體外受精)", ("in vitro fertilization", "IVF", "體外受精", "胚胎植入"), "取卵後在體外與精子受精並培養胚胎，再植入子宮，是 ART 核心技術。"),
    TopicSeed("procedure", "intracytoplasmic-sperm-injection", "Intracytoplasmic sperm injection, ICSI (單一精蟲卵細胞質內注射)", ("intracytoplasmic sperm injection", "ICSI", "單一精蟲卵細胞質內注射"), "將單一精蟲注入卵細胞，常用於 severe male factor infertility 或 IVF fertilization failure。"),
    TopicSeed("condition", "ovarian-hyperstimulation-syndrome", "Ovarian hyperstimulation syndrome, OHSS (卵巢過度刺激症候群)", ("ovarian hyperstimulation syndrome", "OHSS", "卵巢過度刺激症候群"), "排卵誘導後 VEGF-mediated vascular permeability 增加造成卵巢腫大、腹水、血液濃縮、血栓與腎肺併發症。"),
    TopicSeed("condition", "intrauterine-adhesion-asherman-syndrome", "Intrauterine adhesion / Asherman syndrome (子宮腔沾黏)", ("intrauterine adhesion", "Asherman", "子宮腔沾黏"), "子宮內膜基底層損傷後沾黏，可造成月經減少、無月經、不孕或反覆流產；診斷治療常用 hysteroscopy。"),
    TopicSeed("procedure", "hysteroscopy", "Hysteroscopy (子宮鏡)", ("hysteroscopy", "子宮鏡"), "以鏡頭進入子宮腔診斷或治療內膜息肉、黏膜下肌瘤、子宮腔沾黏、隔膜或異常出血。"),
    TopicSeed("condition", "pelvic-organ-prolapse", "Pelvic organ prolapse, POP (骨盆腔器官脫垂)", ("pelvic organ prolapse", "POP", "骨盆腔器官脫垂", "cystocele", "rectocele", "uterine prolapse"), "骨盆底支持缺損造成膀胱、子宮、陰道頂端或直腸膨出，症狀包含墜脹、壓迫、排尿排便與性功能問題。"),
)
...
EIGHTEENTH_BOOK_SEEDS: tuple[TopicSeed, ...] = (
    # Anesthesia evaluation, airway, monitoring, and perioperative physiology.
    TopicSeed("procedure", "preanesthetic-evaluation", "Preanesthetic evaluation (麻醉前評估)", ("preanesthetic evaluation", "麻醉前評估", "麻醉前訪視", "麻醉前的訪視", "麻醉前的訪視重點", "術前診斷", "用藥史"), "麻醉前評估整合病史、用藥、氣道、系統性疾病與手術風險，決定麻醉方式與監測需求。"),
    TopicSeed("diagnostic", "asa-physical-status-classification", "ASA physical status classification (ASA 麻醉風險分級)", ("ASA classification", "ASA physical status", "American Society of Anesthesiology classification", "ASA 分級"), "ASA physical status 以全身健康狀態分級麻醉風險，常用於術前風險溝通與研究分層。"),
    TopicSeed("diagnostic", "mallampati-classification", "Mallampati classification (Mallampati 氣道分級)", ("Mallampati", "Modified Mallampati", "Mallampati classification"), "Mallampati 分級用口咽可視結構估計困難插管風險，是術前氣道評估的一部分。"),
    TopicSeed("procedure", "endotracheal-intubation", "Endotracheal intubation (氣管內插管)", ("endotracheal intubation", "氣管內插管", "插管流程", "endotracheal tube", "ET tube"), "氣管內插管以 laryngoscope 暴露聲門並置入 endotracheal tube，確保通氣、氧合與防止誤吸。"),
    TopicSeed("procedure", "rapid-sequence-intubation", "Rapid sequence intubation, RSI (快速插管)", ("rapid sequence intubation", "RSI", "快速插管", "cricoid pressure"), "RSI 用於高誤吸風險或急症氣道，重點是預氧、快速誘導、肌鬆、避免正壓通氣與確認管位。"),
    TopicSeed("procedure", "laryngeal-mask-airway", "Laryngeal mask airway, LMA (喉罩氣道)", ("laryngeal mask airway", "LMA", "喉罩氣道"), "LMA 是 supraglottic airway，可作為短時間麻醉或困難氣道替代方案，但不等同於完整防誤吸。"),
    TopicSeed("guideline", "difficult-airway-management", "Difficult airway management (困難氣道處理)", ("difficult airway", "困難插管", "困難氣道", "BURP maneuver"), "困難氣道處理需預先評估、準備替代氣道、限制嘗試次數，並保留氧合與外科氣道選項。"),
    TopicSeed("diagnostic", "pulse-oximetry", "Pulse oximetry (脈搏血氧監測)", ("pulse oximetry", "SpO2", "血氧監測", "脈搏血氧"), "Pulse oximetry 以光學方式監測動脈血氧飽和度，是麻醉與急重症基本監測。"),
    TopicSeed("diagnostic", "capnography", "Capnography (二氧化碳波形監測)", ("capnography", "capnogram", "end-tidal CO2", "EtCO2", "二氧化碳監測"), "Capnography 監測呼氣末二氧化碳與波形，可確認氣管管位、通氣與循環狀態。"),
    TopicSeed("diagnostic", "bispectral-index-monitoring", "Bispectral index monitoring, BIS (麻醉深度監測)", ("bispectral index", "BIS", "麻醉深度", "麻醉深度監測"), "BIS 由 EEG 訊號估計催眠深度，用於輔助調整麻醉深度與降低術中覺醒風險。"),
    TopicSeed("procedure", "central-venous-catheterization", "Central venous catheterization (中央靜脈導管置放)", ("central venous catheter", "CVC", "中央靜脈導管", "central line"), "中央靜脈導管用於血管活性藥物、困難周邊靜脈、CVP 估計或大量輸液，但有氣胸、感染與血管損傷風險。"),
    TopicSeed("procedure", "pulmonary-artery-catheterization", "Pulmonary artery catheterization (肺動脈導管)", ("pulmonary artery catheter", "Swan-Ganz", "PA catheter", "肺動脈導管", "PAOP"), "肺動脈導管可量測肺動脈壓、PAOP 與心輸出量，主要用於複雜血流動力學評估。"),
    TopicSeed("concept", "general-anesthesia", "General anesthesia (全身麻醉)", ("general anesthesia", "全身麻醉", "全麻"), "全身麻醉以 hypnosis、analgesia、amnesia、immobility 與 autonomic control 支持手術進行。"),
    TopicSeed("concept", "minimum-alveolar-concentration", "Minimum alveolar concentration, MAC (最小肺泡濃度)", ("minimum alveolar concentration", "MAC", "最小肺泡濃度"), "MAC 是吸入性麻醉劑 potency 指標，受年齡、體溫、電解質、懷孕、酒精與中樞藥物等因素影響。"),
    # Anesthetic drugs and complications.
    TopicSeed("drug", "inhalational-anesthetics", "Inhalational anesthetics (吸入性麻醉劑)", ("inhalational anesthetics", "吸入性麻醉劑", "volatile anesthetics", "氣體麻醉劑"), "吸入性麻醉劑以肺泡吸收與排除，藥效受 MAC、blood/gas partition coefficient 與通氣血流影響。"),
    TopicSeed("drug", "nitrous-oxide", "Nitrous oxide, N2O (笑氣)", ("nitrous oxide", "N2O", "笑氣"), "N2O 起效快、止痛性佳但 potency 低，可擴張封閉氣腔並影響 vitamin B12 相關代謝。"),
    TopicSeed("drug", "sevoflurane", "Sevoflurane (七氟醚)", ("sevoflurane", "七氟醚"), "Sevoflurane 氣味較不刺激，常用於吸入誘導與維持麻醉。"),
    TopicSeed("drug", "desflurane", "Desflurane (地氟醚)", ("desflurane", "地氟醚"), "Desflurane blood/gas 溶解度低、甦醒快，但可刺激氣道與交感反應。"),
    TopicSeed("drug", "isoflurane", "Isoflurane (異氟醚)", ("isoflurane", "異氟醚", "Isoflurance"), "Isoflurane 為常用 volatile anesthetic，具血管擴張與一定心肌抑制作用。"),
    TopicSeed("drug", "halothane", "Halothane (氟烷)", ("halothane", "氟烷"), "Halothane 可造成心肌抑制、心律不整敏感與罕見 halothane hepatitis，現代使用已較少。"),
    TopicSeed("drug", "intravenous-anesthetics", "Intravenous anesthetics (靜脈麻醉劑)", ("intravenous anesthetics", "IV anesthetics", "靜脈麻醉劑", "靜脈麻醉藥"), "靜脈麻醉劑常用於 induction、sedation 或 TIVA，作用受 redistribution、蛋白結合與心血管狀態影響。"),
    TopicSeed("drug", "propofol", "Propofol (丙泊酚)", ("propofol", "丙泊酚"), "Propofol 起效快、甦醒快，可降血壓並抑制呼吸，常用於麻醉誘導與鎮靜。"),
    TopicSeed("drug", "thiopental", "Thiopental (硫噴妥鈉)", ("thiopental", "thiamylal", "methohexital", "barbiturate sodium", "硫噴妥"), "Thiopental 為 barbiturate induction agent，可降低 cerebral metabolic rate 與 ICP，但會造成心血管抑制。"),
    TopicSeed("drug", "ketamine", "Ketamine (氯胺酮)", ("ketamine", "氯胺酮", "NMDA"), "Ketamine 為 dissociative anesthetic，具 analgesia 與交感刺激，常保留呼吸道反射但可增加分泌物與精神症狀。"),
    TopicSeed("drug", "etomidate", "Etomidate (依托咪酯)", ("etomidate", "依托咪酯"), "Etomidate 對血流動力學影響較小，適合不穩定病人誘導，但會抑制 adrenal steroid synthesis。"),
    TopicSeed("drug", "dexmedetomidine", "Dexmedetomidine (右美托咪定)", ("dexmedetomidine", "右美托咪定", "alpha-2 agonist"), "Dexmedetomidine 是 alpha-2 agonist，提供鎮靜與部分止痛，常見 bradycardia 與 hypotension。"),
    TopicSeed("drug", "opioid-analgesics", "Opioid analgesics (鴉片類止痛劑)", ("opioid analgesics", "opioids", "鴉片類藥物", "morphine", "fentanyl", "meperidine"), "Opioids 作用於 opioid receptors，提供強效 analgesia，但可造成 respiratory depression、nausea、ileus 與依賴。"),
    TopicSeed("drug", "naloxone", "Naloxone (納洛酮)", ("naloxone", "納洛酮", "opioid antagonist"), "Naloxone 是 opioid receptor antagonist，可逆轉 opioid-induced respiratory depression，但可能誘發疼痛、戒斷或交感反應。"),
    TopicSeed("drug", "neuromuscular-blocking-agents", "Neuromuscular blocking agents (神經肌肉阻斷劑)", ("neuromuscular blocking agents", "neuromuscular blockers", "肌肉鬆弛劑", "神經肌肉阻斷劑"), "神經肌肉阻斷劑作用於 neuromuscular junction，使骨骼肌鬆弛，需監測與安全逆轉。"),
    TopicSeed("drug", "succinylcholine", "Succinylcholine (琥珀膽鹼)", ("succinylcholine", "琥珀膽鹼", "depolarizing muscle relaxant"), "Succinylcholine 是 depolarizing neuromuscular blocker，起效快但可造成 hyperkalemia、bradycardia、malignant hyperthermia 風險。"),
    TopicSeed("drug", "rocuronium", "Rocuronium (羅庫溴銨)", ("rocuronium", "羅庫溴銨"), "Rocuronium 是 nondepolarizing neuromuscular blocker，可用於快速誘導替代 succinylcholine。"),
    TopicSeed("diagnostic", "neuromuscular-monitoring", "Neuromuscular monitoring (肌鬆監測)", ("neuromuscular monitoring", "train-of-four", "TOF", "肌鬆監測"), "肌鬆監測用 peripheral nerve stimulation 評估 neuromuscular blockade 深度與恢復。"),
    TopicSeed("procedure", "reversal-of-neuromuscular-blockade", "Reversal of neuromuscular blockade (肌鬆逆轉)", ("antagonism", "肌鬆逆轉", "reversal of neuromuscular blockade", "neostigmine"), "肌鬆逆轉以 anticholinesterase 或 sugammadex 等方式恢復肌力，需避免 residual paralysis。"),
    TopicSeed("drug", "local-anesthetics", "Local anesthetics (局部麻醉劑)", ("local anesthetics", "local anesthetic", "局部麻醉劑", "lidocaine", "bupivacaine"), "局部麻醉劑阻斷 voltage-gated sodium channels，造成 sensory、autonomic 與 motor blockade。"),
    TopicSeed("condition", "local-anesthetic-systemic-toxicity", "Local anesthetic systemic toxicity, LAST (局部麻醉劑全身毒性)", ("local anesthetic systemic toxicity", "LAST", "局部麻醉劑中毒", "metallic taste"), "LAST 可造成口周麻木、耳鳴、seizure、心肌抑制與 arrhythmia，需立即支持氣道循環並給 lipid emulsion。"),
    TopicSeed("procedure", "lipid-emulsion-therapy", "Lipid emulsion therapy (脂肪乳劑治療)", ("lipid emulsion", "20% lipid", "脂肪乳劑", "lipid rescue"), "20% lipid emulsion 是 LAST 的關鍵解毒治療，也需同步處理 seizure、hypoxia 與循環不穩。"),
    TopicSeed("procedure", "spinal-anesthesia", "Spinal anesthesia (脊髓麻醉)", ("spinal anesthesia", "脊髓麻醉", "蜘蛛膜下腔麻醉"), "Spinal anesthesia 將局麻藥注入 subarachnoid space，起效快且 blockade 明確，但可能低血壓、尿滯留或 PDPH。"),
    TopicSeed("procedure", "epidural-anesthesia", "Epidural anesthesia (硬膜外麻醉)", ("epidural anesthesia", "硬膜外麻醉", "硬脊膜外麻醉"), "Epidural anesthesia 將藥物給於 epidural space，可連續給藥，用於手術、產痛與術後止痛。"),
    TopicSeed("procedure", "peripheral-nerve-block", "Peripheral nerve block (周邊神經阻斷)", ("peripheral nerve block", "nerve block", "神經阻斷", "臂神經叢阻斷"), "周邊神經阻斷以局麻藥阻斷特定神經或神經叢，提供區域麻醉與術後止痛。"),
    TopicSeed("condition", "post-dural-puncture-headache", "Post-dural puncture headache, PDPH (硬膜穿刺後頭痛)", ("post-dural puncture headache", "PDPH", "low pressure headache", "硬膜穿刺後頭痛"), "PDPH 由 CSF leak 造成姿勢性頭痛，年輕與孕產婦風險較高，可用 hydration、caffeine 或 epidural blood patch。"),
    TopicSeed("procedure", "epidural-blood-patch", "Epidural blood patch (硬膜外自體血液貼片)", ("epidural blood patch", "自體血液凝塊", "硬膜外自體血液"), "Epidural blood patch 以自體血液封閉 dural leak，是持續 PDPH 的有效治療。"),
    TopicSeed("condition", "malignant-hyperthermia", "Malignant hyperthermia (惡性高熱)", ("malignant hyperthermia", "惡性高熱", "ryanodine receptor", "masseter spasm"), "惡性高熱是 volatile anesthetics 或 succinylcholine 誘發的骨骼肌 hypermetabolic crisis，需立即停藥、降溫與 dantrolene。"),
    TopicSeed("drug", "dantrolene", "Dantrolene (丹曲林)", ("dantrolene", "丹曲林"), "Dantrolene 抑制 skeletal muscle sarcoplasmic reticulum calcium release，是 malignant hyperthermia 的特異治療。"),
    TopicSeed("condition", "postoperative-pain", "Postoperative pain (術後疼痛)", ("postoperative pain", "術後疼痛", "手術後疼痛", "multimodal analgesia"), "術後疼痛處理常採 multimodal analgesia，以減少 opioid 用量並促進活動與復原。"),
    TopicSeed("condition", "cancer-pain", "Cancer pain (癌症疼痛)", ("cancer pain", "癌症疼痛", "WHO analgesic ladder"), "癌症疼痛需評估 nociceptive/neuropathic components，常依 WHO analgesic ladder 與輔助藥物調整。"),
    # Ear and vestibular medicine.
    TopicSeed("diagnostic", "tuning-fork-tests", "Tuning fork tests (音叉檢查)", ("Weber test", "Rinne test", "音叉檢查", "Weber", "Rinne"), "Weber/Rinne 音叉檢查可床邊區分 conductive 與 sensorineural hearing loss。"),
    TopicSeed("diagnostic", "pure-tone-audiometry", "Pure tone audiometry, PTA (純音聽力檢查)", ("pure tone audiometry", "PTA", "純音聽力檢查", "pure tone average"), "PTA 測量不同頻率 air/bone conduction threshold，是聽損分類與嚴重度評估核心。"),
    TopicSeed("diagnostic", "tympanometry", "Tympanometry (鼓室圖檢查)", ("tympanometry", "tympanogram", "鼓室圖"), "Tympanometry 評估 tympanic membrane 與 middle-ear compliance，可輔助診斷 OME、鼓膜穿孔或 ossicular chain 問題。"),
    TopicSeed("condition", "conductive-hearing-loss", "Conductive hearing loss (傳導性聽損)", ("conductive hearing loss", "傳導性聽力障礙", "air-bone gap"), "傳導性聽損源自外耳或中耳傳音障礙，常見於 cerumen、otitis media、otosclerosis 或 ossicular disease。"),
    TopicSeed("condition", "sensorineural-hearing-loss", "Sensorineural hearing loss (感音神經性聽損)", ("sensorineural hearing loss", "感音神經性聽力障礙", "SNHL"), "感音神經性聽損源自 cochlea、auditory nerve 或中樞路徑，需依急慢性與伴隨神經症狀評估。"),
    TopicSeed("condition", "sudden-sensorineural-hearing-loss", "Sudden sensorineural hearing loss, SSNHL (突發性耳聾)", ("sudden sensorineural hearing loss", "SSNHL", "突發性耳聾"), "SSNHL 是短時間內發生的感音神經性聽損，常需排除 retrocochlear lesion 並及早治療。"),
    TopicSeed("condition", "cerumen-impaction", "Cerumen impaction (耳垢栓塞)", ("cerumen impaction", "耳垢栓塞"), "耳垢栓塞可造成耳悶、傳導性聽損或耳鳴，處理需避免傷害外耳道或鼓膜。"),
    TopicSeed("condition", "otitis-externa", "Otitis externa (外耳炎)", ("otitis externa", "外耳炎", "swimmer's ear"), "外耳炎為外耳道感染或發炎，常與濕潤、外傷、糖尿病或免疫低下相關。"),
    TopicSeed("condition", "otitis-media-with-effusion", "Otitis media with effusion, OME (積液性中耳炎)", ("otitis media with effusion", "OME", "積液性中耳炎"), "OME 是中耳積液但無急性感染症狀，常造成兒童傳導性聽損與語言學習問題。"),
    TopicSeed("condition", "chronic-otitis-media", "Chronic otitis media, COM (慢性中耳炎)", ("chronic otitis media", "COM", "慢性中耳炎", "慢性化膿性中耳炎"), "慢性中耳炎可有反覆耳漏、鼓膜穿孔與傳導性聽損，需評估 cholesteatoma 與併發症。"),
    TopicSeed("condition", "cholesteatoma", "Cholesteatoma (中耳膽脂瘤)", ("cholesteatoma", "中耳膽脂瘤", "膽脂瘤"), "Cholesteatoma 是角化上皮在中耳或乳突內生長，可侵蝕 ossicles、mastoid、facial nerve 或內耳。"),
    TopicSeed("condition", "otosclerosis", "Otosclerosis (耳硬化症)", ("otosclerosis", "耳硬化症", "stapes fixation"), "Otosclerosis 常造成漸進性 conductive hearing loss，典型為 stapes fixation。"),
    TopicSeed("diagnostic", "caloric-test", "Caloric test (溫差試驗)", ("caloric test", "溫差試驗", "冷熱水試驗"), "Caloric test 以冷熱刺激外耳道評估水平半規管與前庭眼反射功能。"),
    TopicSeed("condition", "peripheral-vertigo", "Peripheral vertigo (周邊型暈眩)", ("peripheral vertigo", "周邊型暈眩", "眩暈"), "周邊型暈眩多源自內耳或 vestibular nerve，常伴 nausea、nystagmus、耳鳴或聽力症狀。"),
    TopicSeed("condition", "benign-paroxysmal-positional-vertigo", "Benign paroxysmal positional vertigo, BPPV (良性陣發性姿勢性眩暈)", ("benign paroxysmal positional vertigo", "BPPV", "良性陣發性姿勢性眩暈"), "BPPV 由耳石移位造成短暫姿勢誘發眩暈，Dix-Hallpike 與 canalith repositioning 是重點。"),
    TopicSeed("condition", "meniere-disease", "Meniere disease (梅尼爾氏症)", ("Meniere", "Ménière", "梅尼爾", "endolymphatic hydrops"), "Meniere disease 典型為反覆眩暈、波動性聽損、耳鳴與耳脹感。"),
    TopicSeed("condition", "vestibular-neuritis", "Vestibular neuritis (前庭神經炎)", ("vestibular neuritis", "前庭神經炎"), "前庭神經炎造成急性持續性周邊眩暈，通常無聽力下降。"),
    # Rhinology, laryngology, neck disease, and head-neck oncology.
    TopicSeed("anatomy", "nasal-cavity", "Nasal cavity (鼻腔)", ("nasal cavity", "鼻腔", "鼻前庭", "固有鼻腔"), "鼻腔由 nasal septum、turbinates、meatus 與鼻竇開口組成，參與加溫、加濕、過濾與嗅覺。"),
    TopicSeed("anatomy", "paranasal-sinuses", "Paranasal sinuses (副鼻竇)", ("paranasal sinus", "paranasal sinuses", "副鼻竇", "鼻竇發育"), "副鼻竇包含 maxillary、frontal、ethmoid、sphenoid sinuses，其開口與感染併發症具臨床重要性。"),
    TopicSeed("condition", "epistaxis", "Epistaxis (鼻出血)", ("epistaxis", "鼻出血", "Kiesselbach"), "鼻出血多為 anterior epistaxis，處理依出血部位、血流動力學與抗凝/凝血狀態調整。"),
    TopicSeed("condition", "nasal-septal-deviation", "Nasal septal deviation (鼻中膈彎曲)", ("nasal septal deviation", "鼻中膈彎曲", "deviated septum"), "鼻中膈彎曲可造成鼻阻塞、鼻出血或鼻竇通氣問題，症狀明顯時可考慮 septoplasty。"),
    TopicSeed("condition", "vasomotor-rhinitis", "Vasomotor rhinitis (血管運動性鼻炎)", ("vasomotor rhinitis", "血管運動性鼻炎"), "血管運動性鼻炎為非過敏性鼻炎，常受溫度、氣味、刺激物或自主神經因素誘發。"),
    TopicSeed("condition", "atrophic-rhinitis", "Atrophic rhinitis (萎縮性鼻炎)", ("atrophic rhinitis", "萎縮性鼻炎", "ozena"), "萎縮性鼻炎造成鼻黏膜萎縮、痂皮與惡臭，需與感染、手術後或全身病因區分。"),
    TopicSeed("condition", "chronic-rhinosinusitis", "Chronic rhinosinusitis (慢性鼻竇炎)", ("chronic rhinosinusitis", "chronic sinusitis", "慢性鼻竇炎", "鼻息肉"), "慢性鼻竇炎是長期鼻竇黏膜發炎，可合併 nasal polyps，治療含藥物與功能性內視鏡鼻竇手術。"),
    TopicSeed("condition", "fungal-sinusitis", "Fungal sinusitis (黴菌性鼻竇炎)", ("fungal sinusitis", "黴菌性鼻竇炎", "fungal ball"), "黴菌性鼻竇炎依 invasive 與 noninvasive 型態有不同風險，免疫低下者需警覺侵襲性病程。"),
    TopicSeed("condition", "sinonasal-mucocele", "Sinonasal mucocele (鼻竇黏液囊腫)", ("mucocele", "鼻竇黏液囊腫", "黏液囊腫"), "鼻竇黏液囊腫因開口阻塞造成黏液擴張，可壓迫眼眶或顱底。"),
    TopicSeed("condition", "cerebrospinal-fluid-rhinorrhea", "Cerebrospinal fluid rhinorrhea (腦脊髓液鼻漏)", ("CSF rhinorrhea", "腦脊髓液鼻漏", "腦脊髓液漏出"), "CSF rhinorrhea 可由外傷、手術或顱底缺損造成，需確認 beta-2 transferrin/glucose 等並預防 meningitis。"),
    TopicSeed("anatomy", "pharynx-larynx", "Pharynx and larynx (咽喉)", ("pharynx", "larynx", "咽部", "喉部", "聲門"), "咽喉包含吞嚥、發聲與氣道保護結構，是 epiglottitis、vocal fold paralysis 與頭頸癌的重要解剖區。"),
    TopicSeed("condition", "vocal-fold-paralysis", "Vocal fold paralysis (聲帶麻痺)", ("vocal fold paralysis", "vocal cord paralysis", "聲帶麻痺"), "聲帶麻痺可造成 hoarseness、aspiration 或 airway compromise，需評估 recurrent laryngeal nerve 與中樞/胸腔病因。"),
    TopicSeed("condition", "laryngomalacia", "Laryngomalacia (喉軟化症)", ("laryngomalacia", "喉軟化症"), "喉軟化症是嬰兒吸氣性 stridor 常見原因，多數隨成長改善，嚴重者需評估餵食與呼吸問題。"),
    TopicSeed("condition", "herpetic-gingivostomatitis", "Herpetic gingivostomatitis (疱疹性齒齦口腔炎)", ("herpetic gingivostomatitis", "疱疹性齒齦口腔炎", "herpetic stomatitis"), "HSV 初感染可造成兒童齒齦口腔炎、發燒、疼痛與水泡/潰瘍。"),
    TopicSeed("condition", "juvenile-nasopharyngeal-angiofibroma", "Juvenile nasopharyngeal angiofibroma (鼻咽血管纖維瘤)", ("nasopharyngeal angiofibroma", "juvenile nasopharyngeal angiofibroma", "鼻咽血管纖維瘤"), "鼻咽血管纖維瘤多見於青春期男性，表現為鼻塞與反覆鼻出血，活檢有大量出血風險。"),
    TopicSeed("condition", "deep-neck-infection", "Deep neck infection (深頸部感染)", ("deep neck infection", "深頸部感染", "深頸部膿瘍"), "深頸部感染可快速擴散至 airway、mediastinum 或 vascular spaces，需評估氣道、影像、抗生素與引流。"),
    TopicSeed("condition", "ludwig-angina", "Ludwig angina (路德維希氏咽峽炎)", ("Ludwig", "Ludwig's angina", "路德維希", "口底蜂窩性組織炎"), "Ludwig angina 是 submandibular/sublingual space 感染，可造成舌根上抬與窒息風險。"),
    TopicSeed("condition", "retropharyngeal-abscess", "Retropharyngeal abscess (咽後膿瘍)", ("retropharyngeal abscess", "咽後膿瘍"), "咽後膿瘍可造成發燒、頸痛、吞嚥困難、stridor，且可向 mediastinum 擴散。"),
    TopicSeed("condition", "sialolithiasis", "Sialolithiasis (唾液腺結石)", ("sialolithiasis", "唾液腺結石", "salivary stone"), "唾液腺結石常見於 submandibular gland，造成進食相關腫痛與反覆感染。"),
    TopicSeed("condition", "salivary-gland-tumor", "Salivary gland tumor (唾液腺腫瘤)", ("salivary gland tumor", "唾液腺腫瘤", "parotid tumor"), "唾液腺腫瘤以 parotid gland 最常見，需依良惡性、facial nerve、影像與細針檢查評估。"),
    TopicSeed("condition", "pleomorphic-adenoma", "Pleomorphic adenoma (多形性腺瘤)", ("pleomorphic adenoma", "多形性腺瘤", "benign mixed tumor"), "Pleomorphic adenoma 是常見良性唾液腺腫瘤，需完整切除以降低復發。"),
    TopicSeed("condition", "warthin-tumor", "Warthin tumor (Warthin 腫瘤)", ("Warthin", "囊狀腺淋巴瘤", "papillary cystadenoma lymphomatosum"), "Warthin tumor 常見於 parotid，與吸菸相關，可雙側或多發。"),
    TopicSeed("condition", "mucoepidermoid-carcinoma", "Mucoepidermoid carcinoma (黏液表皮樣癌)", ("mucoepidermoid carcinoma", "黏液類上皮癌", "黏液表皮樣癌"), "Mucoepidermoid carcinoma 是常見惡性唾液腺腫瘤，預後依 grade 與侵犯程度不同。"),
    TopicSeed("condition", "adenoid-cystic-carcinoma", "Adenoid cystic carcinoma (腺樣囊性癌)", ("adenoid cystic carcinoma", "腺樣囊狀癌", "腺樣囊性癌"), "Adenoid cystic carcinoma 具 perineural invasion 傾向，可能局部復發與遠端轉移。"),
    TopicSeed("procedure", "parotidectomy", "Parotidectomy (腮腺切除術)", ("parotidectomy", "腮腺切除", "superficial parotidectomy"), "Parotidectomy 需保護 facial nerve，常用於 parotid tumor 或特定慢性感染。"),
    TopicSeed("condition", "oral-cavity-cancer", "Oral cavity cancer (口腔癌)", ("oral cavity cancer", "口腔癌", "oral cancer"), "口腔癌多與菸、酒、檳榔相關，治療依 TNM、切除範圍、頸部淋巴與重建需求決定。"),
    TopicSeed("condition", "sinonasal-cancer", "Sinonasal cancer (鼻及鼻竇惡性腫瘤)", ("sinonasal cancer", "nasal and sinus cancer", "鼻及鼻竇惡性腫瘤"), "Sinonasal cancers 罕見且常晚期發現，症狀可類似單側鼻阻塞、鼻出血或眼眶侵犯。"),
    TopicSeed("condition", "nasopharyngeal-carcinoma", "Nasopharyngeal carcinoma, NPC (鼻咽癌)", ("nasopharyngeal carcinoma", "NPC", "鼻咽癌"), "NPC 與 EBV、地理族群與非角化型組織學相關，常以頸部淋巴結、鼻症狀或耳咽管症狀表現。"),
    TopicSeed("condition", "hypopharyngeal-cancer", "Hypopharyngeal cancer (下咽癌)", ("hypopharyngeal cancer", "下咽癌", "梨狀窩"), "下咽癌常晚期診斷，與菸酒相關，可造成吞嚥痛、聲音改變或頸部淋巴結。"),
    TopicSeed("condition", "laryngeal-cancer", "Laryngeal cancer (喉癌)", ("laryngeal cancer", "喉癌", "glottic cancer", "supraglottic"), "喉癌依 supraglottic、glottic、subglottic 位置有不同症狀、淋巴引流與治療策略。"),
    TopicSeed("condition", "oropharyngeal-cancer", "Oropharyngeal cancer (口咽癌)", ("oropharyngeal cancer", "口咽癌", "HPV-mediated", "p16"), "口咽癌需區分 HPV-mediated 與非 HPV，分期與預後差異大。"),
    TopicSeed("guideline", "head-and-neck-cancer-staging", "Head and neck cancer staging (頭頸癌分期)", ("head and neck cancer staging", "TNM staging", "頭頸癌分期", "AJCC 8th"), "頭頸癌分期依部位、T/N/M、HPV/p16 等因素不同，決定治療與預後。"),
    TopicSeed("procedure", "neck-dissection", "Neck dissection (頸部淋巴結廓清術)", ("neck dissection", "頸部淋巴結廓清", "頸部廓清術"), "Neck dissection 依頸部 lymph node levels 與保留結構分類，用於頭頸癌區域控制。"),
)


ALL_SEEDS: tuple[TopicSeed, ...] = SEEDS + SECOND_BOOK_SEEDS + THIRD_BOOK_SEEDS + FOURTH_BOOK_SEEDS + FIFTH_BOOK_SEEDS + SIXTH_BOOK_SEEDS + SEVENTH_BOOK_SEEDS + EIGHTH_BOOK_SEEDS + NINTH_BOOK_SEEDS + TENTH_BOOK_SEEDS + ELEVENTH_BOOK_SEEDS + TWELFTH_BOOK_SEEDS + THIRTEENTH_BOOK_SEEDS + FOURTEENTH_BOOK_SEEDS + FIFTEENTH_BOOK_SEEDS + SIXTEENTH_BOOK_SEEDS + SEVENTEENTH_BOOK_SEEDS + EIGHTEENTH_BOOK_SEEDS
...
def related_links(seed: TopicSeed, available_slugs: set[str]) -> list[str]:
    groups = {
        ...
        "ankle-fracture": ["ottawa-ankle-rules", "open-reduction-internal-fixation", "external-fixation"],
        "ottawa-ankle-rules": ["ankle-fracture", "calcaneus-fracture"],
        "calcaneus-fracture": ["bohler-angle", "open-reduction-internal-fixation"],
        "bohler-angle": ["calcaneus-fracture"],
        "achilles-tendon-rupture": ["orthopedic-rehabilitation", "sports-injury"],
        "anterior-cruciate-ligament-injury": ["acl-reconstruction", "meniscus-tear", "knee-joint", "sports-injury"],
        "posterior-cruciate-ligament-injury": ["anterior-cruciate-ligament-injury", "knee-joint"],
        "meniscus-tear": ["meniscus-repair", "anterior-cruciate-ligament-injury", "osteoarthritis"],
        "acl-reconstruction": ["anterior-cruciate-ligament-injury", "orthopedic-rehabilitation"],
        "meniscus-repair": ["meniscus-tear", "osteoarthritis"],
        "open-fracture": ["gustilo-anderson-open-fracture-classification", "fracture-reduction-fixation", "external-fixation", "surgical-site-infection", "cephalosporins", "aminoglycosides"],
        "gustilo-anderson-open-fracture-classification": ["open-fracture", "external-fixation"],
        "fracture-reduction-fixation": ["open-reduction-internal-fixation", "external-fixation", "open-fracture"],
        "open-reduction-internal-fixation": ["fracture-reduction-fixation", "open-fracture", "distal-radius-fracture"],
        "external-fixation": ["open-fracture", "fracture-reduction-fixation", "pelvic-fracture"],
        "femoral-shaft-fracture": ["open-reduction-internal-fixation", "hemorrhagic-shock", "fat-embolism-syndrome"],
        "tibial-fibular-fracture": ["open-fracture", "compartment-syndrome", "external-fixation"],
        "compartment-syndrome": ["fasciotomy", "tibial-fibular-fracture", "open-fracture"],
        "fasciotomy": ["compartment-syndrome"],
        "femoral-neck-fracture": ["osteoporosis", "osteonecrosis-femoral-head", "open-reduction-internal-fixation"],
        "intertrochanteric-fracture": ["osteoporosis", "open-reduction-internal-fixation"],
        "pelvic-fracture": ["hemorrhagic-shock", "urethral-injury", "external-fixation"],
        "osteonecrosis-femoral-head": ["femoral-neck-fracture", "osteoporosis", "systemic-corticosteroids", "sickle-cell-disease"],
        "shoulder-dislocation": ["brachial-plexus-injury", "orthopedic-rehabilitation"],
        "acromioclavicular-joint-injury": ["shoulder-dislocation", "open-reduction-internal-fixation"],
        "brachial-plexus-injury": ["brachial-plexus", "peripheral-nerves", "shoulder-dislocation"],
        "brachial-plexus": ["brachial-plexus-injury", "peripheral-nerves"],
        "rotator-cuff-injury": ["shoulder-dislocation", "orthopedic-rehabilitation"],
        "supracondylar-humerus-fracture": ["compartment-syndrome", "brachial-plexus-injury", "open-reduction-internal-fixation"],
        "epicondylitis": ["orthopedic-rehabilitation", "nsaids"],
        "elbow-dislocation": ["open-reduction-internal-fixation", "brachial-plexus-injury"],
        "distal-radius-fracture": ["open-reduction-internal-fixation", "carpal-tunnel-syndrome", "osteoporosis"],
        "carpal-tunnel-syndrome": ["median-nerve", "nerve-conduction-study", "peripheral-nerves"],
        "scaphoid-fracture": ["kienbock-disease", "open-reduction-internal-fixation"],
        "kienbock-disease": ["osteonecrosis-femoral-head", "scaphoid-fracture"],
        "extensor-tendon-injury-hand": ["de-quervain-tenosynovitis", "trigger-finger"],
        "de-quervain-tenosynovitis": ["trigger-finger", "nsaids"],
        "trigger-finger": ["de-quervain-tenosynovitis"],
        "bone-metastasis": ["lung-cancer", "breast-cancer", "prostate-cancer", "thyroid-gland", "multiple-myeloma", "spinal-cord-compression"],
        "osteoid-osteoma": ["bone-tumor-location-framework", "bone-tumor-radiographic-signs", "nsaids"],
        "osteoblastoma": ["osteoid-osteoma", "bone-tumor-radiographic-signs"],
        "fibrous-dysplasia": ["bone-tumor-location-framework", "bone-tumor-radiographic-signs", "pathologic-fracture"],
        "osteosarcoma": ["bone-tumor-location-framework", "bone-tumor-radiographic-signs", "chemotherapy", "methotrexate", "doxorubicin", "cisplatin"],
        "osteochondroma": ["bone-tumor-location-framework", "chondrosarcoma"],
        "enchondroma": ["chondrosarcoma", "bone-tumor-radiographic-signs"],
        "chondroblastoma": ["giant-cell-tumor-bone", "bone-tumor-location-framework"],
        "giant-cell-tumor-bone": ["chondroblastoma", "bone-tumor-radiographic-signs"],
        "chondrosarcoma": ["enchondroma", "osteochondroma", "bone-tumor-radiographic-signs"],
        "ewing-sarcoma": ["bone-tumor-radiographic-signs", "chemotherapy", "radiation-therapy"],
        "simple-bone-cyst": ["aneurysmal-bone-cyst", "bone-tumor-radiographic-signs", "pathologic-fracture"],
        "aneurysmal-bone-cyst": ["simple-bone-cyst", "bone-tumor-radiographic-signs"],
        "bone-tumor-location-framework": ["bone-tumor-radiographic-signs", "osteosarcoma", "giant-cell-tumor-bone", "ewing-sarcoma"],
        "bone-tumor-radiographic-signs": ["bone-tumor-location-framework", "osteosarcoma", "ewing-sarcoma", "fibrous-dysplasia"],
        "congenital-radioulnar-synostosis": ["pediatric-developmental-orthopedics"],
        "slipped-capital-femoral-epiphysis": ["developmental-dysplasia-of-hip", "legg-calve-perthes-disease"],
        "developmental-dysplasia-of-hip": ["slipped-capital-femoral-epiphysis", "legg-calve-perthes-disease"],
        "legg-calve-perthes-disease": ["developmental-dysplasia-of-hip", "osteonecrosis-femoral-head"],
        "maternal-physiologic-changes-pregnancy": ["fetal-circulation", "gestational-diabetes", "hypertensive-disorders-of-pregnancy", "urinary-tract-infection"],
        "fetal-circulation": ["maternal-physiologic-changes-pregnancy", "ductus-arteriosus", "cyanotic-congenital-heart-disease"],
        "twin-pregnancy": ["twin-twin-transfusion-syndrome", "preterm-birth", "fetal-growth-restriction"],
        "twin-twin-transfusion-syndrome": ["twin-pregnancy", "umbilical-artery-doppler", "fetal-growth-restriction"],
        "prenatal-care-schedule": ["amniocentesis", "down-syndrome-prenatal-screening", "group-b-streptococcus-screening-pregnancy"],
        "amniocentesis": ["down-syndrome-prenatal-screening", "recurrent-pregnancy-loss"],
        "down-syndrome-prenatal-screening": ["amniocentesis", "prenatal-care-schedule"],
        "group-b-streptococcus-screening-pregnancy": ["puerperal-infection", "neonatal-sepsis", "penicillins"],
        "fetal-growth-restriction": ["umbilical-artery-doppler", "nonstress-test", "biophysical-profile", "preeclampsia"],
        "nonstress-test": ["biophysical-profile", "fetal-heart-rate-decelerations", "fetal-growth-restriction"],
        "biophysical-profile": ["nonstress-test", "umbilical-artery-doppler", "fetal-growth-restriction"],
        "umbilical-artery-doppler": ["fetal-growth-restriction", "preeclampsia"],
        "antepartum-hemorrhage": ["placental-abruption", "placenta-previa", "placenta-accreta-spectrum", "postpartum-hemorrhage"],
        "placental-abruption": ["antepartum-hemorrhage", "disseminated-intravascular-coagulation", "fetal-heart-rate-decelerations"],
        "placenta-previa": ["antepartum-hemorrhage", "placenta-accreta-spectrum", "cesarean-delivery"],
        "placenta-accreta-spectrum": ["placenta-previa", "postpartum-hemorrhage", "cesarean-delivery"],
        "preterm-birth": ["preterm-premature-rupture-of-membranes", "tocolysis", "fetal-fibronectin"],
        "preterm-premature-rupture-of-membranes": ["preterm-birth", "puerperal-infection", "group-b-streptococcus-screening-pregnancy"],
        "fetal-fibronectin": ["preterm-birth", "preterm-premature-rupture-of-membranes"],
        "tocolysis": ["preterm-birth", "magnesium-sulfate-obstetrics", "nsaids", "beta-agonists"],
        "magnesium-sulfate-obstetrics": ["preeclampsia", "eclampsia", "tocolysis"],
        "hypertensive-disorders-of-pregnancy": ["preeclampsia", "eclampsia", "hellp-syndrome", "hypertension"],
        "preeclampsia": ["hypertensive-disorders-of-pregnancy", "eclampsia", "hellp-syndrome", "magnesium-sulfate-obstetrics", "placental-abruption", "fetal-growth-restriction"],
        "eclampsia": ["preeclampsia", "magnesium-sulfate-obstetrics", "seizure"],
        "hellp-syndrome": ["preeclampsia", "disseminated-intravascular-coagulation", "thrombocytopenia"],
        "thyroid-disease-in-pregnancy": ["hyperthyroidism", "hypothyroidism", "thyroid-function-tests"],
        "labor-mechanism": ["bishop-score", "induction-of-labor", "operative-vaginal-delivery"],
        "bishop-score": ["induction-of-labor", "labor-mechanism"],
        "induction-of-labor": ["bishop-score", "oxytocin-obstetrics", "cesarean-delivery"],
        "oxytocin-obstetrics": ["induction-of-labor", "postpartum-hemorrhage"],
        "cesarean-delivery": ["placenta-previa", "placenta-accreta-spectrum", "postpartum-hemorrhage", "puerperal-infection"],
        "operative-vaginal-delivery": ["labor-mechanism", "shoulder-dystocia"],
        "shoulder-dystocia": ["gestational-diabetes", "operative-vaginal-delivery", "brachial-plexus-injury"],
        "fetal-heart-rate-decelerations": ["nonstress-test", "placental-abruption", "umbilical-artery-doppler"],
        "postpartum-hemorrhage": ["placenta-accreta-spectrum", "oxytocin-obstetrics", "amniotic-fluid-embolism", "disseminated-intravascular-coagulation"],
        "amniotic-fluid-embolism": ["postpartum-hemorrhage", "disseminated-intravascular-coagulation", "shock"],
        "puerperal-infection": ["cesarean-delivery", "pelvic-inflammatory-disease", "staphylococcus-aureus-infection"],
        "mastitis-lactational": ["staphylococcus-aureus-infection", "breast-abscess"],
        "female-reproductive-anatomy": ["pelvic-organ-prolapse", "cervical-cancer", "endometrial-cancer", "epithelial-ovarian-cancer"],
        "pelvic-inflammatory-disease": ["tubo-ovarian-abscess", "chlamydia-infection", "gonorrhea", "female-infertility"],
        "tubo-ovarian-abscess": ["pelvic-inflammatory-disease", "ovarian-cyst"],
        "endometriosis": ["endometrial-cancer", "ovarian-cyst", "female-infertility", "epithelial-ovarian-cancer"],
        "adenomyosis": ["abnormal-uterine-bleeding", "endometriosis"],
        "uterine-leiomyoma": ["abnormal-uterine-bleeding", "hysteroscopy", "female-infertility"],
        "abnormal-uterine-bleeding": ["uterine-leiomyoma", "endometrial-hyperplasia", "endometrial-cancer", "amenorrhea"],
        "endometrial-hyperplasia": ["endometrial-cancer", "polycystic-ovary-syndrome", "abnormal-uterine-bleeding"],
        "endometrial-cancer": ["endometrial-hyperplasia", "abnormal-uterine-bleeding", "obesity", "polycystic-ovary-syndrome"],
        "ovarian-cyst": ["ovarian-torsion", "mature-cystic-teratoma", "endometriosis", "tubo-ovarian-abscess"],
        "ovarian-torsion": ["ovarian-cyst", "mature-cystic-teratoma"],
        "epithelial-ovarian-cancer": ["ca-125", "ovarian-cancer-debulking-surgery", "endometriosis", "krukenberg-tumor"],
        "ovarian-germ-cell-tumor": ["mature-cystic-teratoma", "alpha-fetoprotein", "human-chorionic-gonadotropin"],
        "ovarian-sex-cord-stromal-tumor": ["amenorrhea", "abnormal-uterine-bleeding", "estrogen"],
        "mature-cystic-teratoma": ["ovarian-germ-cell-tumor", "ovarian-torsion"],
        "krukenberg-tumor": ["epithelial-ovarian-cancer", "gastric-cancer", "colorectal-cancer"],
        "ovarian-cancer-debulking-surgery": ["epithelial-ovarian-cancer", "chemotherapy"],
        "ca-125": ["epithelial-ovarian-cancer", "endometriosis", "tumor-markers"],
        "human-papillomavirus-infection": ["hpv-vaccine", "cervical-intraepithelial-neoplasia", "cervical-cancer", "vaginal-cancer", "vulvar-cancer"],
        "hpv-vaccine": ["human-papillomavirus-infection", "cervical-cancer", "vaccine-contraindications"],
        "cervical-intraepithelial-neoplasia": ["human-papillomavirus-infection", "pap-smear", "colposcopy", "cervical-conization", "cervical-cancer"],
        "cervical-cancer": ["human-papillomavirus-infection", "cervical-intraepithelial-neoplasia", "pap-smear", "colposcopy", "cervical-conization", "radiation-therapy"],
        "pap-smear": ["human-papillomavirus-infection", "cervical-intraepithelial-neoplasia", "cervical-cancer"],
        "colposcopy": ["pap-smear", "cervical-intraepithelial-neoplasia", "cervical-cancer"],
        "cervical-conization": ["cervical-intraepithelial-neoplasia", "cervical-cancer", "preterm-birth"],
        "vaginal-cancer": ["human-papillomavirus-infection", "cervical-cancer"],
        "vulvar-cancer": ["human-papillomavirus-infection", "cervical-cancer"],
        "gestational-trophoblastic-disease": ["choriocarcinoma", "human-chorionic-gonadotropin", "methotrexate"],
        "choriocarcinoma": ["gestational-trophoblastic-disease", "chemotherapy", "methotrexate"],
        "amenorrhea": ["primary-ovarian-insufficiency", "polycystic-ovary-syndrome", "hyperprolactinemia", "female-infertility"],
        "primary-ovarian-insufficiency": ["amenorrhea", "osteoporosis", "female-infertility"],
        "menopause": ["menopausal-hormone-therapy", "osteoporosis", "abnormal-uterine-bleeding"],
        "menopausal-hormone-therapy": ["menopause", "osteoporosis", "venous-thromboembolism", "breast-cancer"],
        "recurrent-pregnancy-loss": ["amniocentesis", "antiphospholipid-syndrome", "intrauterine-adhesion-asherman-syndrome"],
        "female-infertility": ["infertility-evaluation", "hysterosalpingography", "assisted-reproductive-technology", "endometriosis", "polycystic-ovary-syndrome", "male-infertility"],
        "infertility-evaluation": ["female-infertility", "male-infertility", "semen-analysis", "hysterosalpingography"],
        "hysterosalpingography": ["female-infertility", "intrauterine-adhesion-asherman-syndrome"],
        "assisted-reproductive-technology": ["in-vitro-fertilization", "intracytoplasmic-sperm-injection", "ovarian-hyperstimulation-syndrome", "male-infertility", "female-infertility"],
        "in-vitro-fertilization": ["assisted-reproductive-technology", "intracytoplasmic-sperm-injection", "ovarian-hyperstimulation-syndrome"],
        "intracytoplasmic-sperm-injection": ["in-vitro-fertilization", "male-infertility"],
        "ovarian-hyperstimulation-syndrome": ["assisted-reproductive-technology", "polycystic-ovary-syndrome", "venous-thromboembolism"],
        ...
        "preanesthetic-evaluation": ["asa-physical-status-classification", "mallampati-classification", "difficult-airway-management", "general-anesthesia"],
        "asa-physical-status-classification": ["preanesthetic-evaluation", "general-anesthesia"],
        "mallampati-classification": ["preanesthetic-evaluation", "difficult-airway-management", "endotracheal-intubation"],
        "endotracheal-intubation": ["rapid-sequence-intubation", "laryngeal-mask-airway", "difficult-airway-management", "capnography", "airways"],
        "rapid-sequence-intubation": ["endotracheal-intubation", "succinylcholine", "rocuronium", "aspiration-pneumonia"],
        "laryngeal-mask-airway": ["endotracheal-intubation", "difficult-airway-management"],
        "difficult-airway-management": ["mallampati-classification", "endotracheal-intubation", "laryngeal-mask-airway", "tracheostomy"],
        "pulse-oximetry": ["oxygenation-and-ventilation", "capnography", "general-anesthesia"],
        "capnography": ["endotracheal-intubation", "mechanical-ventilation", "oxygenation-and-ventilation"],
        "bispectral-index-monitoring": ["general-anesthesia", "intravenous-anesthetics", "inhalational-anesthetics"],
        "central-venous-catheterization": ["fluid-resuscitation", "shock", "pneumothorax"],
        "pulmonary-artery-catheterization": ["heart-failure", "pulmonary-hypertension", "shock"],
        "general-anesthesia": ["preanesthetic-evaluation", "intravenous-anesthetics", "inhalational-anesthetics", "endotracheal-intubation", "neuromuscular-blocking-agents"],
        "minimum-alveolar-concentration": ["inhalational-anesthetics", "general-anesthesia"],
        "inhalational-anesthetics": ["minimum-alveolar-concentration", "nitrous-oxide", "sevoflurane", "desflurane", "isoflurane", "halothane", "malignant-hyperthermia"],
        "nitrous-oxide": ["inhalational-anesthetics"],
        "sevoflurane": ["inhalational-anesthetics", "malignant-hyperthermia"],
        "desflurane": ["inhalational-anesthetics", "malignant-hyperthermia"],
        "isoflurane": ["inhalational-anesthetics", "malignant-hyperthermia"],
        "halothane": ["inhalational-anesthetics", "malignant-hyperthermia"],
        "intravenous-anesthetics": ["general-anesthesia", "propofol", "thiopental", "ketamine", "etomidate", "dexmedetomidine", "benzodiazepines"],
        "propofol": ["intravenous-anesthetics", "general-anesthesia"],
        "thiopental": ["intravenous-anesthetics", "raised-intracranial-pressure"],
        "ketamine": ["intravenous-anesthetics", "asthma", "raised-intracranial-pressure"],
        "etomidate": ["intravenous-anesthetics", "adrenal-insufficiency"],
        "dexmedetomidine": ["intravenous-anesthetics", "bradycardia"],
        "opioid-analgesics": ["postoperative-pain", "cancer-pain", "naloxone", "opioid-intoxication", "opioid-withdrawal"],
        "naloxone": ["opioid-analgesics", "opioid-intoxication"],
        "neuromuscular-blocking-agents": ["neuromuscular-junction", "succinylcholine", "rocuronium", "neuromuscular-monitoring", "reversal-of-neuromuscular-blockade", "myasthenia-gravis"],
        "succinylcholine": ["neuromuscular-blocking-agents", "malignant-hyperthermia", "hyperkalemia"],
        "rocuronium": ["neuromuscular-blocking-agents", "rapid-sequence-intubation"],
        "neuromuscular-monitoring": ["neuromuscular-blocking-agents", "reversal-of-neuromuscular-blockade"],
        "reversal-of-neuromuscular-blockade": ["neuromuscular-blocking-agents", "neuromuscular-monitoring"],
        "local-anesthetics": ["local-anesthetic-systemic-toxicity", "spinal-anesthesia", "epidural-anesthesia", "peripheral-nerve-block", "epidural-blood-patch"],
        "local-anesthetic-systemic-toxicity": ["local-anesthetics", "lipid-emulsion-therapy", "seizure", "arrhythmia"],
        "lipid-emulsion-therapy": ["local-anesthetic-systemic-toxicity", "local-anesthetics"],
        "spinal-anesthesia": ["local-anesthetics", "post-dural-puncture-headache", "epidural-anesthesia"],
        "epidural-anesthesia": ["local-anesthetics", "post-dural-puncture-headache", "epidural-blood-patch", "labor-pain"],
        "peripheral-nerve-block": ["local-anesthetics", "brachial-plexus", "postoperative-pain"],
        "post-dural-puncture-headache": ["spinal-anesthesia", "epidural-anesthesia", "epidural-blood-patch"],
        "epidural-blood-patch": ["post-dural-puncture-headache", "epidural-anesthesia"],
        "malignant-hyperthermia": ["inhalational-anesthetics", "succinylcholine", "dantrolene", "rhabdomyolysis"],
        "dantrolene": ["malignant-hyperthermia"],
        "postoperative-pain": ["opioid-analgesics", "peripheral-nerve-block", "local-anesthetics"],
        "cancer-pain": ["opioid-analgesics", "palliative-care"],
        "tuning-fork-tests": ["pure-tone-audiometry", "conductive-hearing-loss", "sensorineural-hearing-loss"],
        "pure-tone-audiometry": ["tuning-fork-tests", "tympanometry", "conductive-hearing-loss", "sensorineural-hearing-loss", "sudden-sensorineural-hearing-loss"],
        "tympanometry": ["otitis-media-with-effusion", "conductive-hearing-loss", "acute-otitis-media"],
        "conductive-hearing-loss": ["tuning-fork-tests", "pure-tone-audiometry", "cerumen-impaction", "acute-otitis-media", "otitis-media-with-effusion", "chronic-otitis-media", "otosclerosis"],
        "sensorineural-hearing-loss": ["pure-tone-audiometry", "sudden-sensorineural-hearing-loss", "meniere-disease", "congenital-cmv-infection"],
        "sudden-sensorineural-hearing-loss": ["sensorineural-hearing-loss", "pure-tone-audiometry"],
        "cerumen-impaction": ["conductive-hearing-loss", "otitis-externa"],
        "otitis-externa": ["cerumen-impaction", "conductive-hearing-loss"],
        "acute-otitis-media": ["otitis-media-with-effusion", "conductive-hearing-loss", "tympanometry"],
        "otitis-media-with-effusion": ["acute-otitis-media", "tympanometry", "conductive-hearing-loss"],
        "chronic-otitis-media": ["cholesteatoma", "conductive-hearing-loss", "otitis-media-with-effusion"],
        "cholesteatoma": ["chronic-otitis-media", "conductive-hearing-loss", "facial-nerve-palsy"],
        "otosclerosis": ["conductive-hearing-loss", "pure-tone-audiometry"],
        "caloric-test": ["peripheral-vertigo", "vestibular-neuritis", "meniere-disease"],
        "peripheral-vertigo": ["benign-paroxysmal-positional-vertigo", "meniere-disease", "vestibular-neuritis", "caloric-test"],
        "benign-paroxysmal-positional-vertigo": ["peripheral-vertigo"],
        "meniere-disease": ["peripheral-vertigo", "sensorineural-hearing-loss", "caloric-test"],
        "vestibular-neuritis": ["peripheral-vertigo", "caloric-test"],
        "nasal-cavity": ["paranasal-sinuses", "epistaxis", "nasal-septal-deviation", "allergic-rhinitis"],
        "paranasal-sinuses": ["nasal-cavity", "acute-bacterial-sinusitis", "chronic-rhinosinusitis", "fungal-sinusitis", "sinonasal-mucocele", "sinonasal-cancer"],
        "epistaxis": ["nasal-cavity", "juvenile-nasopharyngeal-angiofibroma", "hypertension"],
        "nasal-septal-deviation": ["nasal-cavity", "chronic-rhinosinusitis"],
        "allergic-rhinitis": ["vasomotor-rhinitis", "atrophic-rhinitis", "chronic-rhinosinusitis"],
        "vasomotor-rhinitis": ["allergic-rhinitis", "atrophic-rhinitis"],
        "atrophic-rhinitis": ["vasomotor-rhinitis", "nasal-cavity"],
        "acute-bacterial-sinusitis": ["paranasal-sinuses", "chronic-rhinosinusitis", "cavernous-sinus-thrombosis"],
        "chronic-rhinosinusitis": ["paranasal-sinuses", "acute-bacterial-sinusitis", "fungal-sinusitis", "sinonasal-mucocele"],
        "fungal-sinusitis": ["chronic-rhinosinusitis", "paranasal-sinuses", "immunosuppressants"],
        "sinonasal-mucocele": ["paranasal-sinuses", "chronic-rhinosinusitis"],
        "cerebrospinal-fluid-rhinorrhea": ["paranasal-sinuses", "skull-fracture", "meningitis"],
        "pharynx-larynx": ["epiglottitis", "vocal-fold-paralysis", "laryngomalacia", "laryngeal-cancer", "hypopharyngeal-cancer"],
        "epiglottitis": ["pharynx-larynx", "airways", "endotracheal-intubation"],
        "vocal-fold-paralysis": ["pharynx-larynx", "recurrent-laryngeal-nerve-injury", "thyroidectomy"],
        "laryngomalacia": ["pharynx-larynx", "stridor"],
        "herpetic-gingivostomatitis": ["herpes-simplex-virus-infection", "acyclovir"],
        "juvenile-nasopharyngeal-angiofibroma": ["epistaxis", "nasopharyngeal-carcinoma"],
        "deep-neck-infection": ["ludwig-angina", "retropharyngeal-abscess", "airways", "mediastinitis"],
        "ludwig-angina": ["deep-neck-infection", "airways"],
        "retropharyngeal-abscess": ["deep-neck-infection", "mediastinitis"],
        "sialolithiasis": ["salivary-gland-tumor"],
        "salivary-gland-tumor": ["pleomorphic-adenoma", "warthin-tumor", "mucoepidermoid-carcinoma", "adenoid-cystic-carcinoma", "parotidectomy"],
        "pleomorphic-adenoma": ["salivary-gland-tumor", "parotidectomy"],
        "warthin-tumor": ["salivary-gland-tumor", "smoking"],
        "mucoepidermoid-carcinoma": ["salivary-gland-tumor"],
        "adenoid-cystic-carcinoma": ["salivary-gland-tumor", "peripheral-nerves"],
        "parotidectomy": ["salivary-gland-tumor", "facial-nerve-palsy"],
        "oral-cavity-cancer": ["head-and-neck-cancer-staging", "neck-dissection", "radiation-therapy"],
        "sinonasal-cancer": ["paranasal-sinuses", "head-and-neck-cancer-staging"],
        "nasopharyngeal-carcinoma": ["head-and-neck-cancer-staging", "epstein-barr-virus-infection", "radiation-therapy"],
        "hypopharyngeal-cancer": ["pharynx-larynx", "head-and-neck-cancer-staging", "laryngeal-cancer"],
        "laryngeal-cancer": ["pharynx-larynx", "head-and-neck-cancer-staging", "hypopharyngeal-cancer"],
        "oropharyngeal-cancer": ["human-papillomavirus-infection", "head-and-neck-cancer-staging", "neck-dissection"],
        "head-and-neck-cancer-staging": ["oral-cavity-cancer", "nasopharyngeal-carcinoma", "hypopharyngeal-cancer", "laryngeal-cancer", "oropharyngeal-cancer", "neck-dissection"],
        "neck-dissection": ["head-and-neck-cancer-staging", "oral-cavity-cancer", "nasopharyngeal-carcinoma"],
    }

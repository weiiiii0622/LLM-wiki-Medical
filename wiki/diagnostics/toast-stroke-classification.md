---
type: diagnostic
status: draft
created: 2026-06-08
updated: 2026-06-08
sources:
  - "[[sources/med4-book4-ch05]]"
aliases:
  - 'TOAST stroke classification (TOAST 中風病因分類)'
  - 'TOAST stroke classification'
  - 'TOAST 中風病因分類'
tags:
  - medicine
  - diagnostic
  - auto-topic-node
---

# TOAST stroke classification (TOAST 中風病因分類)

## Summary

急性缺血性中風病因分類，包含 large artery atherosclerosis、cardioembolism、small vessel 等。 本頁已整合至 `醫(四)第4冊神內`；若同一 topic 也出現在舊來源，來源與重點會保留並合併。

## Source Coverage

- [[sources/med4-book4-ch05]]

## Key Details From Sources

- #### B. TOAST 急性缺血性中風的病因分類 Source: [[sources/med4-book4-ch05|戊、腦血管疾病]].

## Clinical Caveats

- 本頁是 textbook-derived study note，不可直接作為診療、處方或急症處置依據。
- 若問題涉及 current guideline、drug dosing、禁忌、pregnancy/lactation、兒科、renal/hepatic adjustment 或高風險處置，需查最新版官方來源。

## Related Pages

- [[conditions/ischemic-stroke|Ischemic stroke (缺血性腦中風)]]

## Follow-up

- 後續可補上 guideline 年份、治療流程圖、diagnostic criteria 表格與藥物安全監測整理。

...
- (1) 血中氧含量(oxygen content)包括化學結合和物理溶解兩部分。 - a. 計算公式: Hgb×1.3×SpO2(%) + 0.003×PaO2(mmHg) - b. 以動脈及靜脈的狀況計算血氧含量: Source: [[sources/med3-book1-ch22|甲、胸腔科基本知識]].
- ...患者休息狀態時,動脈血已出現缺氧現象,該病患較不可能 因 COPD 而出現下列那一項呼吸生理的障礙? 醫三-46 (A) 第一秒吐氣量 (FEV,) 低於預期值的 50% (B) 分流 (shunt) 增加 (C) 換氣 / 灌流失衡 (ventilation/perfusion mismatching) (D) 第一秒吐氣量(FEV1)低於預期值的25%時,可能同時伴有動 脈血中二氧化碳升高(PaCO,) 【詳解】(B)分流(shunt)增加發生於肺炎等無法進行氧氣交換的 區域。 105(二)專 (D) 4. 下列有關呼吸衰竭 (respiratory failure) 之敘述,何者錯誤? (A) 一般以動脈血中的氣體(如氧氣、二氧化碳)分壓為判斷標準 醫三-49 (B) 肺纖維化 (id Source: [[sources/med3-book1-ch22|甲、胸腔科基本知識]].
